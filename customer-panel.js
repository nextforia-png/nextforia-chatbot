...
module.exports = function renderCustomerPanel(res, options) {
  ...
  const personalityPath = options.personalityPath || "/admin/panel/bot-personality";
  ...
  // Conversations is the single shared inbox for both bots. Appointment-only
  // tenants can open it from a concrete appointment/reminder without gaining
  // access to the support-only summary, orders or sales views.
  const supportTabs = panelContext.support
    ? ["summary", "conversations", "human", "retargeting"]
    : (panelContext.appointments ? ["conversations"] : []);
...
.appointment-bot-view .botSwitch{gap:7px}.appointment-bot-view .botCard{border-radius:12px;padding:10px 11px}.appointment-bot-view .nav{gap:5px}.appointment-bot-view #navAppointments .navItem{height:auto;min-height:43px;border-radius:11px;padding:10px 13px;font-size:13.5px;line-height:1.2}.appointment-bot-view #navAppointments .navItem.active{background:#fff;color:#0587CC;box-shadow:0 6px 18px rgba(2,9,23,.18)}.appointment-bot-view #navAppointments .navItem.active .navIcon{color:#00A0F0}
...
.notificationCard{position:relative;background:#fff;border:1px solid var(--line);border-radius:20px;padding:20px 48px 20px 20px;display:grid;grid-template-columns:auto minmax(0,1fr) auto;gap:16px;align-items:start;box-shadow:var(--shadow)}
...
.notificationDismiss{position:absolute;top:10px;right:10px;width:28px;height:28px;display:grid;place-items:center;padding:0;border:0;border-radius:9px;background:transparent;color:#7B8AA4;font-family:inherit;font-size:21px;line-height:1;cursor:pointer}.notificationDismiss:hover{background:#EEF4F9;color:#0A1836}.notificationDismiss:focus-visible{outline:3px solid rgba(18,168,244,.24);outline-offset:2px}
...
.handoffToast.show{display:grid;animation:handoffIn .24s ease-out}.handoffToastIcon{width:46px;height:46px;border-radius:15px;background:#F7A928;color:#fff;display:grid;place-items:center;font-size:23px}.handoffToast strong{display:block;font-size:15px;font-weight:950}.handoffToast p{margin-top:3px;color:#6B7280;font-size:12.5px;line-height:1.4;font-weight:700}.handoffToast #handoffToastButton{border:0;border-radius:12px;background:var(--navy-900);color:#fff;padding:11px 13px;font-weight:900;cursor:pointer}.handoffToastClose{position:absolute;top:7px;right:8px;width:28px;height:28px;display:grid;place-items:center;padding:0!important;border:0!important;border-radius:9px!important;background:transparent!important;color:#7B8AA4!important;font-family:inherit;font-size:21px;line-height:1;cursor:pointer}.handoffToastClose:hover{background:#F4ECDD!important;color:#0A1836!important}.handoffToastClose:focus-visible{outline:3px solid rgba(18,168,244,.24);outline-offset:2px}
...
@media(max-width:760px){
  ...
  .panel-redesign .mobileTabbar{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));overflow:visible;height:72px;padding:7px 10px}.panel-redesign .mobileTabbar button{display:grid;min-width:0;height:58px}.panel-redesign .mobileTabbar button.active{border-radius:14px;background:#EAF8FF;color:#057BB6;box-shadow:inset 0 3px 0 #0587CC}.panel-redesign .mobileTabbar .navBadge{position:absolute;top:4px;right:calc(50% - 24px)}
}
          ...
          <section class="notificationEmailSettings" id="notificationEmailSettings" hidden>
            ...
            <div class="notificationEmailDetails" id="notificationEmailDetails" hidden>
              ...
              <fieldset class="notificationEmailChoices"><legend>¿Qué quieres que vigilemos por ti?</legend><div class="notificationEmailTypes">
                ...
                <label class="notificationEmailType"><input type="checkbox" data-notification-email-type="appointment_confirmed"><span class="notificationEmailTypeCheck" aria-hidden="true"></span><span class="notificationEmailTypeCopy"><strong>Citas confirmadas</strong><small>Nuevas reservas listas en tu agenda</small></span></label>
              </div></fieldset>
...
function mobileGoHome(){
  if(state.bot==="appointments"&&PANEL_CONTEXT.appointments){showTab("appointments");if(typeof showAppointmentSection==="function")showAppointmentSection("panel");return;}
...
function mobileGoChats(){
  if(state.bot==="appointments"&&PANEL_CONTEXT.appointments){if(typeof openAppointmentConversations==="function")openAppointmentConversations();return;}
...
function updateMobilePrimaryNav(){
  ...
  var appointmentChat=state.tab==="conversations"&&state.bot==="appointments";
...
function showTab(name){
  ...
  if(PANEL_CONTEXT.v2&&["summary","orders","retargeting"].includes(name)&&!PANEL_CONTEXT.support)name="appointments";
  state.tab=name;
  ...
  if(name==="summary"||name==="orders"||name==="retargeting")state.bot="support";
  if(name==="conversations"&&!(state.bot==="appointments"&&PANEL_CONTEXT.appointments))state.bot="support";
  syncBotSidebar();
  ...
  var supportModule=name==="summary"||(name==="conversations"&&state.bot!=="appointments")||name==="orders",setupModule=name==="setup"||name==="notifications",retargetingModule=name==="retargeting",appointmentsModule=name==="appointments"||(name==="conversations"&&state.bot==="appointments");
...
function dismissedNotificationIds(){try{var value=JSON.parse(localStorage.getItem("nextfor_dismissed_notifications")||"[]");return Array.isArray(value)?value.map(String):[];}catch(_){return[];}}
function notificationIsDismissed(id){return !!id&&dismissedNotificationIds().includes(String(id));}
function rememberNotificationDismissal(id){if(!id)return;var ids=dismissedNotificationIds().filter(function(value){return value!==String(id);});ids.unshift(String(id));try{localStorage.setItem("nextfor_dismissed_notifications",JSON.stringify(ids.slice(0,100)));}catch(_){}}
function notificationUnreadTotal(items,pending){return (items||[]).filter(function(item){return ["human_handoff_required","customer_order_created","appointment_created"].includes(item.type)&&!item.read&&!notificationIsDismissed(item.id);}).length+Number(pending||0);}
function markNotificationReadLocally(id){if(!id||!state.notifications)return;(state.notifications.items||[]).forEach(function(item){if(item.id===id)item.read=true;});state.notifications.unread_count=notificationUnreadTotal(state.notifications.items,state.notifications.pending_count);renderNextforNotifications();}
function dismissNotification(id){if(!id)return;rememberNotificationDismissal(id);if(state.notifications)state.notifications.unread_count=notificationUnreadTotal(state.notifications.items,state.notifications.pending_count);renderNextforNotifications();fetch("/admin/panel/notifications/"+encodeURIComponent(id)+"/read",{method:"POST",headers:{accept:"application/json","x-nextforia-panel-origin":location.origin},keepalive:true}).catch(function(){});}
...
function renderNextforNotifications(){var root=document.getElementById("nextforNotifications"),notifications=state.notifications||{},items=(notifications.items||[]).filter(function(item){return !notificationIsDismissed(item.id);}),pending=Number(notifications.pending_count)||0,unread=notificationUnreadTotal(items,pending);["navNotificationCount","mnavNotificationCount","mobileHeaderNotificationCount"].forEach(function(id){var el=document.getElementById(id);if(el){el.textContent=unread||"";el.style.display=unread?"grid":"none";}});if(!root)return;if(!items.length){root.innerHTML='<article class="notificationEmpty">No tienes notificaciones pendientes. Las nuevas citas, pedidos y solicitudes de ayuda aparecerán aquí inmediatamente.</article>';return;}root.innerHTML=items.map(function(item){var questions=(item.pending_questions||[]).slice(0,6).map(function(question){return '<span>'+esc(question.label||question.path)+'</span>';}).join(""),actionUrl=String(item.action_url||""),itemId=String(item.id||""),action=actionUrl?'<button class="primaryBtn" type="button" data-action-url="'+attr(actionUrl)+'" data-notification-id="'+attr(itemId)+'" onclick="event.stopPropagation();notificationAction(this.dataset.actionUrl,this.dataset.notificationId)">'+esc(item.action_label||"Abrir")+'</button>':'',high=item.priority==="high",liveType=["human_handoff_required","customer_order_created","appointment_created"].includes(item.type),unreadClass=liveType&&!item.read?" unread":"",clickAttrs=actionUrl?' clickable" role="button" tabindex="0" data-action-url="'+attr(actionUrl)+'" data-notification-id="'+attr(itemId)+'" onclick="notificationAction(this.dataset.actionUrl,this.dataset.notificationId)" onkeydown="notificationCardKey(event,this)':'"',icon=item.type==="human_handoff_required"?"🙋":(item.type==="customer_order_created"?"📦":(item.type==="appointment_created"?"📅":(high?PANEL_CHECK_ICON:"✧")));return '<article class="notificationCard '+(high?"high":"")+unreadClass+clickAttrs+'><span class="notificationIcon">'+icon+'</span><div class="notificationCopy"><h4>'+esc(item.title||"Notificación")+'</h4><p>'+esc(item.message||"")+'</p>'+(questions?'<div class="pendingQuestionList">'+questions+'</div>':"")+'</div><button class="notificationDismiss" type="button" data-notification-id="'+attr(itemId)+'" onclick="event.stopPropagation();dismissNotification(this.dataset.notificationId)" aria-label="Cerrar notificación">×</button><div>'+action+'</div></article>';}).join("");if(pending)renderPendingSetupReminder(pending);}
...
try{var initialView=new URL(location.href).searchParams.get("view");if(["panel","agenda","reminders","rules"].includes(initialView))state.appointmentSection=initialView;}catch(e){}
