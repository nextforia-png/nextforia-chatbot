...
module.exports = function renderCustomerPanel(res, options) {
  ...
  // Orders is presentation-only until the commercial backend publishes a real,
  // tenant-scoped capability. Keep it in the public demo without exposing demo
  // data or a dead module to signed-in customers.
  const ordersEnabled = panelRedesignEnabled && demoMode;
  const availableTabs = ["summary", "conversations", "human", "appointments", "plan", "channels", "setup", "notifications", "retargeting", "tests"].concat(ordersEnabled ? ["orders"] : []);
...
.mobileBrand{display:flex;align-items:center;gap:10px;min-width:0}
  ...
  .areaChart{height:168px;margin-top:8px;padding-bottom:18px}.chartStats{display:flex;gap:26px;margin-top:14px;padding-bottom:2px}.chartStats div{display:flex;flex-direction:column;gap:2px}.chartStats strong{font-family:var(--display,inherit);font-size:20px;font-weight:800;color:#0A1836;line-height:1}.chartStats span{font-size:11.5px;font-weight:600;color:#64748B}.areaChart{position:relative}.activityMarks{position:absolute;inset:0;pointer-events:none}.activityValue{position:absolute;font-size:12px;font-weight:800;color:#0A1836;background:rgba(255,255,255,.92);border:1px solid #DFE6F0;border-radius:999px;padding:2px 7px;white-space:nowrap;line-height:1.2}.activityDay{position:absolute;bottom:-2px;font-size:11px;font-weight:600;color:#94A3B8;white-space:nowrap}.activityDot{fill:#fff;stroke:#12A8F4;stroke-width:3;vector-effect:non-scaling-stroke}@media(max-width:860px){.chartStats{gap:16px}.chartStats strong{font-size:17px}.activityValue{font-size:11px;padding:1px 5px}.activityDay{font-size:10px}}
...
@media(max-width:760px){
  ...
  .panel-redesign .metricRow{grid-template-columns:1fr 1fr;gap:10px}.panel-redesign .metric{min-height:156px;padding:16px;border-radius:16px}.panel-redesign .metricValue{font-size:32px}.panel-redesign #kSales{font-size:27px}
      ...
      ${ordersEnabled ? `<section class="${viewClass('panel-orders')}" id="panel-orders">
        <div class="ordersView">
          <section class="ordersToolbar">
            <div class="searchBox"><span class="searchIcon" aria-hidden="true">⌕</span><input id="orderSearch" aria-label="Buscar pedidos" placeholder="Buscar por pedido, cliente o teléfono" oninput="renderOrders()"></div>
            <div class="orderFilters" aria-label="Filtros de pedidos"><button class="active" type="button" data-order-filter="all" onclick="setOrderFilter(this)">Todos</button><button type="button" data-order-filter="por_confirmar" onclick="setOrderFilter(this)">Por confirmar</button><button type="button" data-order-filter="pagado" onclick="setOrderFilter(this)">Pagados</button><button type="button" data-order-filter="preparacion" onclick="setOrderFilter(this)">Preparación</button><button type="button" data-order-filter="enviado" onclick="setOrderFilter(this)">Enviados</button><button type="button" data-order-filter="cancelado" onclick="setOrderFilter(this)">Cancelados</button></div>
          </section>
          ...
          <p class="ordersDisclosure">Datos de ejemplo para revisión visual del panel-demo. No representan pedidos reales.</p>
        </div>
      </section>` : ""}
      ...
      ${channelConnectionsV1Enabled ? `<section class="${viewClass('panel-channels')}" id="panel-channels">
        <div class="channelsView">
          ...
          <div class="channelWorkspace">
            ...
            ${panelRedesignEnabled ? '<aside class="channelLivePreview"><div class="previewTabs"><button class="active" type="button" onclick="setChannelPreview(\'whatsapp\',this)">WhatsApp</button><button type="button" onclick="setChannelPreview(\'instagram\',this)">Instagram</button></div><div class="previewPhone"><div class="previewPhoneHead"><span id="previewPhoneIcon">◉</span><div><strong id="previewPhoneTitle">WhatsApp</strong><small>Vista previa en vivo</small></div></div><div class="previewChat"><span class="previewCustomer">Hola, ¿me ayudas a elegir?</span><span class="previewBot">¡Claro! Cuéntame qué buscas y te ayudo enseguida.</span></div><div class="previewComposer">Escribe un mensaje… <b>➤</b></div></div><p>Así verán tus clientes una conversación con tu Nextfor.</p></aside>' : ""}
          </div>
...
function setChannelPreview(channel,button){document.querySelectorAll(".previewTabs button").forEach(function(item){item.classList.toggle("active",item===button);});text("previewPhoneTitle",channel==="instagram"?"Instagram":"WhatsApp");text("previewPhoneIcon",channel==="instagram"?"◎":"◉");}
...
var ORDER_STAGE_LABELS={por_confirmar:"Por confirmar",pagado:"Pagado",preparacion:"En preparación",enviado:"Enviado",cancelado:"Cancelado"};
...
function orderChannelGlyph(channel){return channel==="whatsapp"?"◉":channel==="instagram"?"◎":"●";}
function setOrderFilter(button){state.orderFilter=button&&button.dataset&&button.dataset.orderFilter||"all";document.querySelectorAll(".orderFilters [data-order-filter]").forEach(function(item){var active=item===button;item.classList.toggle("active",active);item.setAttribute("aria-pressed",active?"true":"false");});renderOrders();}
function selectOrder(id){state.selectedOrder=Number(id);renderOrders();}
function updateDemoOrder(id,stage){var order=state.orders.find(function(item){return item.id===Number(id);});if(!order)return;order.stage=stage;renderOrders();panelToast("Pedido demo actualizado a "+(ORDER_STAGE_LABELS[stage]||stage)+". No se modificó información real.");}
function demoOrderPlaceholder(){panelToast("Acción demo: el conector real definirá el siguiente estado.");}
function demoTransportPlaceholder(){panelToast("Próximamente: integración con transportadora.");}
function renderOrders(){if(!PANEL_REDESIGN_ENABLED)return;var list=document.getElementById("orderListPane"),detail=document.getElementById("orderDetailPane");if(!list||!detail)return;var query=String(document.getElementById("orderSearch")&&document.getElementById("orderSearch").value||"").trim().toLowerCase(),rows=state.orders.filter(function(order){var match=state.orderFilter==="all"||order.stage===state.orderFilter;var haystack=[order.id,order.name,order.phone].join(" ").toLowerCase();return match&&(!query||haystack.includes(query));});list.innerHTML='<header class="orderListHead"><div><strong>'+rows.length+' pedidos</strong><span>Ejemplos del panel-demo</span></div><span class="demoBadge">DEMO</span></header><div class="orderRows">'+(rows.length?rows.map(function(order){return '<button class="orderRow'+(state.selectedOrder===order.id?' active':'')+'" type="button" data-order="'+order.id+'" onclick="selectOrder(this.dataset.order)"><span class="orderAvatar">'+esc(order.name.split(" ").slice(0,2).map(function(part){return part.charAt(0);}).join(""))+'<i class="'+attr(order.channel)+'">'+orderChannelGlyph(order.channel)+'</i></span><span class="orderRowMain"><span><strong>#'+order.id+' · '+esc(order.name)+'</strong><b>'+cop(orderTotal(order))+'</b></span><small>'+esc(order.phone)+' · '+esc(order.location)+'</small><em class="orderStage '+attr(order.stage)+'">'+esc(ORDER_STAGE_LABELS[order.stage]||order.stage)+'</em></span></button>';}).join(""):'<div class="orderNoResults">No hay pedidos con este filtro.</div>')+'</div>';var order=state.orders.find(function(item){return item.id===state.selectedOrder;})||rows[0];if(!order){detail.innerHTML='<span class="orderDetailEyebrow">Detalle del pedido</span><h3>Selecciona un pedido</h3>';return;}state.selectedOrder=order.id;var subtotal=orderTotal(order)-(Number(order.shipping)||0);detail.innerHTML='<header class="orderDetailHead"><div><span class="orderDetailEyebrow">Pedido #'+order.id+'</span><h3>'+esc(order.name)+'</h3></div><span class="orderStage '+attr(order.stage)+'">'+esc(ORDER_STAGE_LABELS[order.stage]||order.stage)+'</span></header><div class="orderTimeline"><i class="done"></i><i class="'+(["pagado","preparacion","enviado"].includes(order.stage)?"done":"")+'"></i><i class="'+(["preparacion","enviado"].includes(order.stage)?"done":"")+'"></i><i class="'+(order.stage==="enviado"?"done":"")+'"></i></div><section class="orderDetailCard"><small>Cliente y entrega</small><strong>'+esc(order.phone)+'</strong><p>'+esc(order.address)+' · '+esc(order.location)+'</p></section><section class="orderProducts"><small>Productos</small>'+(order.items||[]).map(function(item){return '<div><span>'+item.qty+' × '+esc(item.name)+'</span><strong>'+cop(item.price*item.qty)+'</strong></div>';}).join("")+'<div><span>Envío</span><strong>'+cop(order.shipping)+'</strong></div><div class="orderTotal"><span>Total</span><strong>'+cop(orderTotal(order))+'</strong></div></section><section class="orderDetailCard"><small>Pago</small><strong>'+esc(order.payment)+'</strong><p>'+esc(order.paymentNote)+'</p></section>'+(order.guide?'<section class="orderDetailCard"><small>Guía</small><strong>'+esc(order.guide)+'</strong></section>':'')+'<div class="orderDetailActions">'+(order.stage==="por_confirmar"?'<button type="button" data-id="'+order.id+'" data-stage="pagado" onclick="updateDemoOrder(this.dataset.id,this.dataset.stage)">Confirmar pago</button>':'<button type="button" onclick="demoOrderPlaceholder()">Actualizar estado</button>')+'<button class="secondary" type="button" onclick="demoTransportPlaceholder()">Enviar a transportadora · Pronto</button></div>';
}
...
function renderActivity(items){var box=document.getElementById("activityChart");if(!box)return;if(!items.length){items=[34,43,58,37,74,88,61].map(function(messages,index){return{day:String(index),messages:messages};});}
var vals=items.map(function(i){return i.messages||0;});var max=Math.max.apply(null,vals)||1;var n=items.length;
var total=vals.reduce(function(a,b){return a+b;},0);var avg=Math.round(total/(n||1));var peak=Math.max.apply(null,vals);
var pts=items.map(function(i,idx){var x=n===1?350:idx*(700/(n-1));var y=190-((i.messages||0)/max*150);return [x,y];});
...
var marks=items.map(function(item,idx){
  ...
  return '<span class="activityValue" style="left:'+xp.toFixed(2)+'%;top:'+yp.toFixed(2)+'%;transform:translate('+shift+',-150%)">'+esc(item.messages||0)+'</span>'+
...
text("activityRange","+18% vs. período anterior");
...
function setOpportunityFilter(filter){state.opportunityFilter=filter;renderDemoOpportunities();}
