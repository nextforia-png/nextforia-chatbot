...
.channelMark{display:inline-flex;align-items:center;gap:5px;border-radius:999px;padding:4px 8px;font-size:10.5px;font-weight:900;white-space:nowrap}
.channelMark:before{content:"";width:6px;height:6px;border-radius:50%;background:currentColor}
.channelMark.whatsapp{background:#EAF8F1;color:#137A4D}
.channelMark.instagram{background:#FFF0F7;color:#B12E72}
...
<div class="app">
  ...
  <main class="main">
    <header class="topbar">
      <div class="pageTitle"><h2 id="pageTitle">Resumen</h2><p id="pageSubtitle">Resultados del bot de atención · Últimos 7 días</p></div>
...
function renderChannelState(){if(!state.data)return;var channels=state.data.business&&state.data.business.channels||{},wa=channels.whatsapp||{},ig=channels.instagram||{},readyCount=(wa.status==="ready"?1:0)+(ig.status==="ready"?1:0),ready=readyCount>0,total=(wa.conversations_count||0)+(ig.conversations_count||0),status=document.getElementById("moduleStatus-support");text("moduleStatus-support",readyCount+" canales activos");if(status)status.classList.toggle("off",!ready);text("mobileModule-support","Atención al cliente · "+(ready?"Activo":"Pendiente"));text("channelStatusTitle",ready?"Bot de atención conectado":"Configuración pendiente");text("channelStatusDetail","WhatsApp + Instagram · "+total+" conversaciones visibles");var search=document.getElementById("conversationSearch");if(search)search.placeholder="Buscar por nombre, @usuario, teléfono o mensaje";var dot=document.getElementById("channelStatusDot");if(dot)dot.style.background=ready?"#22C778":"#F5A524";}
...
function sendReply(){var item=findConversation(state.selected),input=document.getElementById("replyText"),message=input?input.value.trim():"";if(!item||!SERVER_CAPABILITIES.respond)return;if(!message){text("chatStatus","Escribe un mensaje antes de enviar.");return;}setSendBusy(true);api("/admin/send-message",{method:"POST",body:JSON.stringify({userId:conversationKey(item),text:message})}).then(function(){if(input)input.value="";updateReplyCount();text("chatStatus","Mensaje enviado por "+(item.channel_label||"el canal del cliente")+".");loadPanelData(false);}).catch(function(error){text("chatStatus","No se pudo enviar: "+error.message);}).finally(function(){setSendBusy(false);});}
