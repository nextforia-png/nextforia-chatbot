...
function showView(name){if(!titles[name])return;currentView=name;document.querySelectorAll(".nav-button").forEach(function(el){var active=el.dataset.view===name;el.classList.toggle("active",active);el.setAttribute("aria-current",active?"page":"false");});document.querySelectorAll(".view").forEach(function(el){el.classList.toggle("active",el.dataset.panel===name);});document.getElementById("pageTitle").textContent=titles[name][0];document.getElementById("pageSubtitle").textContent=titles[name][1];try{history.replaceState(null,"","/admin/super-admin"+(name==="overview"?"":"?view="+encodeURIComponent(name)));}catch(e){}if(name==="agendamiento")loadAppointmentOverview();document.querySelector(".content").scrollTop=0;}
...
function startSetupAppointmentCalendarConnection(tenantId){postJson("/admin/appointment-calendar-connections/"+encodeURIComponent(tenantId)+"/connect",{}).then(function(body){if(body.authorization_url)location.href=body.authorization_url;else showToast("No llegó URL de Google.");}).catch(function(error){showToast(error.message==="calendar_oauth_not_configured"?"Falta configurar OAuth de Google Calendar.":"No se pudo iniciar Google Calendar.");});}
function verifySetupAppointmentCalendar(tenantId){postJson("/admin/appointment-calendar-connections/"+encodeURIComponent(tenantId)+"/verify",{}).then(function(){showToast("Calendario verificado.");return openSetupReview(tenantId);}).catch(function(){showToast("No se pudo verificar Google Calendar.");});}
...
if(review.status==="ready"&&answersApproved&&(answers.setup_goal==="appointments"||answers.setup_goal==="both")&&!record.appointment_configuration){var buildAppt=el("button","button primary","Construir configuración Appointment");buildAppt.type="button";buildAppt.addEventListener("click",function(){saveSetupReview("build_configuration","appointments");});actions.appendChild(buildAppt);}
if(review.status==="building"&&(record.customer_service_configuration||record.appointment_configuration)){var saveConfig=el("button","button primary","Guardar configuración");saveConfig.type="button";saveConfig.addEventListener("click",function(){saveSetupReview("save_configuration");});var approveConfig=el("button","button","Aprobar para Testing");approveConfig.type="button";approveConfig.addEventListener("click",function(){saveSetupReview("approve_configuration");});actions.append(saveConfig,approveConfig);}
if(review.status==="testing"&&(record.customer_service_configuration||record.appointment_configuration)){var editConfig=el("button","button","Guardar cambios y volver a Building");editConfig.type="button";editConfig.addEventListener("click",function(){saveSetupReview("save_configuration");});actions.appendChild(editConfig);var wantsCalls=record.answers&&record.answers.channels&&record.answers.channels.phone_calls;if(record.appointment_configuration&&(record.appointment_configuration.external_status!=="configured"||(wantsCalls&&record.appointment_configuration.external_phone_status!=="configured"))){var configureAgent=el("button","button primary","Configurar agente real");configureAgent.type="button";configureAgent.addEventListener("click",function(){saveSetupReview("configure_appointment_agent","appointments");});actions.appendChild(configureAgent);}}
if(record.setup_completed&&["customer_service","appointments","both"].includes(answers.setup_goal)&&review.status!=="live"){var launch=el("button","button primary",body.launch&&body.launch.ready?"Aceptar y activar Live":"No se puede activar aún");launch.type="button";launch.disabled=!(body.launch&&body.launch.ready);launch.addEventListener("click",function(){saveSetupReview("launch_live");});actions.appendChild(launch);}
