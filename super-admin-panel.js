...
function renderSuperAdminPanel(res, options) {
  ...
  function renderLeadRows(rows) {
    ...
    return rows.map(function (row) {
      return '<div class="lead-row"><div><strong>' + escapeHtml(row.company_name || row.tenant_id) + '</strong><small>' + escapeHtml(row.tenant_id || "—") + '</small></div><div><strong>' + escapeHtml(row.admin_email || "—") + '</strong><small>' + escapeHtml(row.contact_phone || "Sin teléfono") + '</small></div><div><span class="badge ' + leadBadge(row.stage) + ' dot">' + escapeHtml(row.stage_label || "Lead") + '</span><small>' + num(row.completion || 0) + '% setup</small></div><div><strong>' + escapeHtml(row.plan_id || "Plan inicial") + '</strong><small>' + escapeHtml(row.assigned_bot_id || "Bot inicial") + '</small></div><div><strong>' + escapeHtml(leadDisplayDate(row)) + '</strong><small>' + (row.updated_at ? 'actualizado' : 'creado') + '</small></div><div><small>' + escapeHtml(row.next_action || "Revisar lead.") + '</small></div></div>';
    }).join("");
  }
...
function setupChannelStatus(detail,channel,requested){var connection=setupChannelConnection(detail,channel),status=connection&&connection.status;if(status==="connected")return{label:"Conectado",tone:"success",detail:connection.account_label||"Validado técnicamente."};if(status==="connecting")return{label:"Pendiente de conexión",tone:"warning",detail:"Autorización iniciada; falta terminar la conexión."};if(status==="needs_attention")return{label:"Requiere revisión",tone:"danger",detail:connection.last_error||"La conexión necesita atención técnica."};if(requested)return{label:"Pendiente de conexión",tone:"warning",detail:"El cliente lo informó durante el setup. Falta activarlo técnicamente."};return{label:"No solicitado",tone:"neutral",detail:"El cliente no dejó datos para este canal en el setup."};}
function requestedSetupChannels(detail){var answers=detail&&detail.onboarding&&detail.onboarding.answers||{},items=[];function add(channel,label,value,requested){var state=setupChannelStatus(detail,channel,requested);items.push({channel:channel,label:label,value:value||"—",state:state});}
var whatsapp=answers.meta&&answers.meta.whatsapp_number||"";add("whatsapp","WhatsApp",whatsapp,!!whatsapp||answers.meta&&answers.meta.whatsapp_integration_intent==="yes");
...
var commerce=answers.commerce&&answers.commerce.platform||"";var commerceRequested=commerce&&commerce!=="none"&&commerce!=="unknown"||answers.commerce&&answers.commerce.integration_intent==="yes";items.push({channel:"commerce",label:"Commerce",value:commerceRequested?((commerce||"Tienda")+" "+(answers.commerce&&answers.commerce.store_url?("· "+answers.commerce.store_url):"")):"—",state:commerceRequested?{label:"Pendiente de conexión",tone:"warning",detail:"El cliente informó una tienda o intención de integración. Falta validar acceso técnico."}:{label:"No solicitado",tone:"neutral",detail:"El cliente no solicitó integración de tienda."}});
return items;}
...
function setupReviewActionForCheck(check,tenantId){if(!check||!tenantId)return null;if(check.code==="whatsapp_connection_required")return{label:"Conectar WhatsApp",run:function(){startSetupChannelConnection(tenantId,"whatsapp");}};if(check.code==="whatsapp_connected")return{label:"Verificar WhatsApp",run:function(){verifySetupChannel(tenantId,"whatsapp");}};if(check.code==="shopify_connection_required")return{label:"Conectar Shopify",run:function(){location.href="/admin/integrations/shopify/connect/"+encodeURIComponent(tenantId);}};if(check.code==="shopify_connected")return{label:"Abrir Shopify",run:function(){location.href="/admin/integrations/shopify/connect/"+encodeURIComponent(tenantId);}};if(check.code==="required_setup_answers_missing"||check.code==="setup_not_completed"||check.code==="setup_changes_requested")return{label:"Editar setup",run:function(){document.querySelector("#setupReviewDetail [data-review-path]")&&document.querySelector("#setupReviewDetail [data-review-path]").focus();}};return null;}
function renderLaunchReadiness(root,detail){var launch=detail&&detail.launch||{},tenantId=launch.tenant_id||detail&&detail.tenant&&detail.tenant.id,section=el("section","callout "+(launch.ready?"info-callout":""));section.style.marginTop="0";var wrap=el("div");wrap.appendChild(el("strong",null,launch.ready?"Listo para activar Live":"Antes de activar Live"));wrap.appendChild(el("p",null,launch.ready?"Cuando des Aceptar, Super Admin aprueba, deja configuración lista y activa el cliente.":"El sistema revisa lo real antes de activar. Corrige estos puntos y luego podrás dar Aceptar."));var list=el("div","launch-checklist");(launch.checks||[]).forEach(function(check){var item=el("div","launch-check "+(check.type||"ok"));item.appendChild(el("span","launch-dot",check.type==="blocker"?"!":check.type==="warning"?"?":"✓"));var text=el("div");text.appendChild(el("strong",null,check.label||check.code));text.appendChild(el("span",null,check.detail||""));var action=setupReviewActionForCheck(check,tenantId);if(action){var btn=el("button","button",action.label);btn.type="button";btn.addEventListener("click",action.run);text.appendChild(btn);}item.appendChild(text);list.appendChild(item);});if(!(launch.checks||[]).length){var empty=el("div","launch-check warning");empty.appendChild(el("span","launch-dot","?"));var emptyText=el("div");emptyText.appendChild(el("strong",null,"Sin validación todavía"));emptyText.appendChild(el("span",null,"Abre de nuevo este setup para recalcular la checklist."));empty.appendChild(emptyText);list.appendChild(empty);}wrap.appendChild(list);section.appendChild(wrap);section.appendChild(el("span","badge "+(launch.ready?"success":"warning"),launch.ready?"Puede ir Live":"Faltan pasos"));root.appendChild(section);}
...
function setupReviewErrorLabel(code){return({unauthorized:"Tu sesión expiró. Vuelve a entrar.",tenant_not_found:"Ese cliente ya no existe.",setup_not_completed:"El cliente todavía no ha terminado el setup.",setup_must_be_approved:"Primero aprueba el setup antes de construir.",customer_service_not_selected:"Este setup no incluye Customer Service. Las citas pertenecen al bot de agendamiento.",configuration_not_building:"Primero genera el borrador de Customer Service.",configuration_required:"La configuración está incompleta.",launch_confirmation_required:"Confirma la activación Live antes de continuar.",launch_blocked:"Todavía faltan pasos reales antes de activar Live.",setup_review_unavailable:"No se pudo cargar o guardar la revisión."})[code]||"No se pudo completar la revisión.";}
...
if(record.setup_completed&&answers.setup_goal==="customer_service"&&review.status!=="live"){var launch=el("button","button primary",body.launch&&body.launch.ready?"Aceptar y activar Live":"No se puede activar aún");launch.type="button";launch.disabled=!(body.launch&&body.launch.ready);launch.addEventListener("click",function(){saveSetupReview("launch_live");});actions.appendChild(launch);}
