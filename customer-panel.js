...
function channelConnectionInitial(channel){return({whatsapp:"WA",instagram:"IG",messenger:"MS",telegram:"TG",calendar:"GC","google-calendar":"GC"})[channel]||"NX";}
...
function renderAppointmentCalendarCard(calendar,canManage){if(!calendar)return"";var status=calendar.status||"not_connected",connected=status==="connected",connectable=calendar.authorization_available!==false&&(calendar.connect_available||status==="not_connected"||status==="disconnected"||status==="needs_attention"),account=calendar.calendar_summary||calendar.account_label||calendar.account_email||"",actions='<span class="channelState '+attr(status)+'">'+esc(calendarConnectionStatusLabel(status))+'</span>';if(canManage){if(connectable&&!connected)actions+='<button class="primaryBtn" type="button" onclick="connectAppointmentCalendar()">Conectar calendario</button>';else if(calendar.authorization_available===false)actions+='<span class="channelAccount">Estamos preparando Google Calendar.</span>';if(connected)actions+='<button class="ghostBtn" type="button" onclick="connectAppointmentCalendar()">Cambiar calendario</button>';if(calendar.disconnect_available||connected)actions+='<button class="ghostBtn" type="button" onclick="disconnectAppointmentCalendar()">Desconectar</button>';}return '<article class="channelConnectCard recommended"><span class="channelConnectIcon calendar">'+esc(channelConnectionInitial("google-calendar"))+'</span><div class="channelConnectCopy"><h4>'+esc(calendar.name||"Google Calendar")+'</h4><p>'+esc(calendar.description||"Conecta el calendario donde Nextfor debe revisar disponibilidad y registrar citas.")+'</p><div class="channelAccount">'+esc(account||"Necesario para agendar sin cruces")+'</div></div><div class="channelConnectActions">'+actions+'</div></article>';}
function renderChannelConnections(){var root=document.getElementById("channelConnectionCards"),payload=state.channelConnections;if(!root||!payload)return;var canManage=SERVER_ROLE==="admin"||SERVER_ROLE==="super_admin",available=payload.meta_authorization_available||{},hints=selectedChannelHints(onboardingAnswers()),cards=(payload.channels||[]).map(function(item){var channel=item.channel||item.id,status=item.status||"not_connected",soon=item.coming_soon||item.available===false,recommended=hints.includes(channel),account=item.account_label?'<div class="channelAccount">'+esc(item.account_label)+'</div>':recommended?'<div class="channelAccount">Sugerido por tu cuestionario</div>':"",historyHint=channel==="whatsapp"&&status!=="connected"?'<div class="channelAccount">Para traer tus conversaciones anteriores, elige “Compartir chats” cuando Meta te lo pregunte. WhatsApp Business seguirá funcionando en tu celular.</div>':"",primary=channel==="whatsapp",actions='<span class="channelState '+attr(status)+'">'+esc(soon?"Próximamente":channelConnectionStatusLabel(status))+'</span>';if(!soon&&canManage){if(item.requires_selection){var options=(item.pending_assets||[]).map(function(asset){return '<option value="'+attr(asset.id)+'">'+esc(asset.label+(asset.detail?" · "+asset.detail:""))+'</option>';}).join("");actions+='<select class="channelAssetSelect" id="channelAsset-'+attr(channel)+'" aria-label="Elige una cuenta">'+options+'</select><button class="primaryBtn" type="button" data-channel="'+attr(channel)+'" onclick="selectChannelAsset(this.dataset.channel)">Elegir esta cuenta</button>';}else if(item.connect_available){actions+='<button class="'+(primary?"primaryBtn":"ghostBtn")+'" type="button" data-channel="'+attr(channel)+'" onclick="connectChannel(this.dataset.channel)">'+(available[channel]===false?"Estamos preparando este paso":"Continuar con Meta")+'</button>';}else if(item.reconnect_available){actions+='<button class="'+(primary?"primaryBtn":"ghostBtn")+'" type="button" data-channel="'+attr(channel)+'" onclick="connectChannel(this.dataset.channel)">Volver a conectar</button>';}else if(channel==="whatsapp"&&status==="connected"){actions+='<button class="ghostBtn" type="button" data-channel="whatsapp" onclick="connectChannel(this.dataset.channel)">Completar conexión con Meta</button>';}if(item.disconnect_available){actions+='<button class="ghostBtn" type="button" data-channel="'+attr(channel)+'" data-name="'+attr(item.name||channel)+'" onclick="disconnectChannel(this.dataset.channel,this.dataset.name)">Desconectar</button>';}}return '<article class="channelConnectCard'+(soon?" comingSoon":"")+(primary?" primaryChannel":"")+(recommended&&!soon?" recommended":"")+'"><span class="channelConnectIcon '+attr(channel)+'">'+esc(channelConnectionInitial(channel))+'</span><div class="channelConnectCopy"><h4>'+esc(item.name||channel)+'</h4><p>'+esc(item.description||"")+'</p>'+account+historyHint+'</div><div class="channelConnectActions">'+actions+'</div></article>';});cards.push(renderAppointmentCalendarCard(payload.appointment_calendar,canManage));root.innerHTML=cards.join("");renderConnectionHub();}
...
function connectAppointmentCalendar(){if(DEMO_MODE){setChannelConnectionMessage("Demo: aquí abriríamos Google para conectar el calendario de tu negocio.","success");return;}var externalTab=prepareExternalIntegrationTab("Google Calendar");if(!externalTab){setChannelConnectionMessage("Tu navegador bloqueó la nueva pestaña. Permite ventanas emergentes para Nextfor y vuelve a intentar.","error");return;}setChannelConnectionMessage("Google Calendar se abrirá en una pestaña nueva…");api("/admin/panel/appointment-calendar/google/connect",{method:"POST",body:JSON.stringify({return_to:"channels"})}).then(function(body){if(!body.authorization_url)throw new Error("authorization_unavailable");if(!navigateExternalIntegrationTab(externalTab,body.authorization_url))throw new Error("popup_navigation_failed");setChannelConnectionMessage("Google Calendar se abrió en una pestaña nueva. Termina allí y luego vuelve a este panel.","success");}).catch(function(error){closeExternalIntegrationTab(externalTab);setChannelConnectionMessage(error.body&&error.body.message||"No pudimos abrir Google Calendar. Intenta de nuevo o habla con NextforIA.","error");loadChannelConnections(true);});}
