...
try{var url=new URL(location.href),requested=url.searchParams.get("view");if(requested&&titles[requested])showView(requested);if(url.searchParams.has("key")){url.searchParams.delete("key");history.replaceState(null,"",url.pathname+url.search+url.hash);}}catch(e){}loadHealth();loadPlatformGoals().catch(function(){});loadLeadPipeline();loadQuestionnaire();loadSetupReviews();if(customerAccessV2Enabled){loadCustomerInvitations();bindCatalogForms();bindTenantDelete();loadCatalog();loadTenants();}if(paymentsV1Enabled)loadBillingAdmin();
</script></body></html>`);
}
