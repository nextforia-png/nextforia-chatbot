      ...
      ${panelRedesignEnabled ? `<section class="${viewClass('panel-orders')}" id="panel-orders">
        <div class="ordersView">
          <section class="ordersToolbar">
            ...
            <div class="orderFilters" aria-label="Filtros de pedidos"><button class="active" type="button" data-order-filter="all" onclick="setOrderFilter(this)">Todos</button><button type="button" data-order-filter="pending" onclick="setOrderFilter(this)">Por confirmar</button><button type="button" data-order-filter="confirmed" onclick="setOrderFilter(this)">Confirmados</button></div>
          </section>
          <div class="ordersShell">
            <section class="orderListPane"><div class="orderEmptyIcon">${PANEL_ICONS.package}</div><h3 id="orderEmptyTitle">Tus pedidos aparecerán aquí</h3><p id="orderEmptyCopy">Cuando una tienda o canal conectado entregue pedidos al panel, podrás revisarlos sin salir de Nextfor.</p><span class="ordersTruthBadge" id="orderEmptyBadge">Sin pedidos disponibles</span></section>
...
function setOrderFilter(button){var value=button&&button.dataset&&button.dataset.orderFilter||"all",copy={all:["Tus pedidos aparecerán aquí","Cuando una tienda o canal conectado entregue pedidos al panel, podrás revisarlos sin salir de Nextfor.","Sin pedidos disponibles"],pending:["No hay pedidos por confirmar","Cuando un cliente envíe un comprobante o cierre una compra, aparecerá aquí para revisión manual.","0 por confirmar"],confirmed:["No hay pedidos confirmados","Los pagos confirmados aparecerán aquí antes de pasar a preparación y envío.","0 confirmados"]}[value]||[];state.orderFilter=value;document.querySelectorAll(".orderFilters [data-order-filter]").forEach(function(item){var active=item===button;item.classList.toggle("active",active);item.setAttribute("aria-pressed",active?"true":"false");});text("orderEmptyTitle",copy[0]);text("orderEmptyCopy",copy[1]);text("orderEmptyBadge",copy[2]);}
