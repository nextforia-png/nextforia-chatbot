...
(async function run() {
  ...
  try {
    ...
    assert.strictEqual(checkoutA.amount_charged, 299900);
    ...
    assert.strictEqual(payload.billing.provider_fee_type, "estimated");
	    assert.strictEqual(payload.billing.provider_fee, 8997);
	    assert.strictEqual(payload.billing.net_amount, 290903);
    ...
    response = await fetch(base + "/admin/panel/billing/checkout", {
      ...
      body: JSON.stringify({ plan_id: "nextfor-uno", bot_id: "atencion-cliente" })
    });
    ...
    assert.strictEqual(adminA.net_amount, 290903);
