...
module.exports = function renderCustomerPanel(res, options) {
  ...
  const PAGE_TITLES = { summary: "Resumen", conversations: "Conversaciones", appointments: "Citas", plan: "Mi plan", channels: "Conecta WhatsApp", setup: "Configuración de tu Nextfor IA", retargeting: "Seguimientos comerciales", tests: "Pruebas" };
  const PAGE_SUBTITLES = { summary: summarySubtitle, conversations: "La IA atiende y te deja solo lo que necesita de ti.", appointments: "Tu agenda llenándose, sin perseguir confirmaciones.", plan: "Plan, módulos y consumo", channels: "Empieza por el canal donde más te escriben", setup: "Tu negocio, tu voz y tus reglas en un solo lugar", retargeting: "Cola segura, aprobaciones, cancelaciones y auditoría", tests: "Herramientas seguras para validar el bot." };
      ...
      ${channelConnectionsV1Enabled ? `<section class="${viewClass('panel-channels')}" id="panel-channels">
        <div class="channelsView">
          <section class="channelsHero">
            <h3>Conecta donde tus clientes te contactan</h3>
            <p>Empieza con WhatsApp. El botón abre la autorización oficial de Meta para elegir el número o cuenta correcta; NextforIA nunca te pedirá tokens, claves ni credenciales técnicas.</p>
            <div class="metaConnectionSteps" aria-label="Pasos simples de conexión con Meta">
              <article class="metaConnectionStep"><b>1</b><strong>Entrar con Meta</strong><span>Inicias sesión en la pantalla oficial de Meta.</span></article>
              <article class="metaConnectionStep"><b>2</b><strong>Elegir la cuenta</strong><span>Seleccionas el número de WhatsApp o la cuenta de Instagram correcta.</span></article>
              <article class="metaConnectionStep"><b>3</b><strong>Queda verificado</strong><span>NextforIA confirma permisos y webhooks antes de usar el canal.</span></article>
            </div>
          </section>
        ...
        <div class="setupView">
          <section class="setupSummaryPanel">
            ...
            <div class="setupFlowSteps">
              ...
              <article class="setupFlowStep next"><b>3</b><strong>Conectar WhatsApp</strong><span>Instagram queda como opción adicional.</span></article>
            ...
            <div class="channelPlan"><div><h4>Próximo paso: conectar WhatsApp</h4><p id="channelConnectionSummary">${channelConnectionsV1Enabled ? "Conecta WhatsApp con la autorización oficial de Meta. Instagram es opcional y puedes continuar más tarde." : "La conexión de canales todavía no está disponible para esta cuenta."}</p></div>${channelConnectionsV1Enabled ? '<button class="primaryBtn" type="button" onclick="showTab(\'channels\')">Conectar WhatsApp</button>' : '<button class="primaryBtn" type="button" disabled>Conexión pendiente</button>'}</div>
...
function channelConnectionStatusLabel(status){return({not_connected:"No conectado",connecting:"Conectando",connected:"Conectado",needs_attention:"Necesita atención",disconnected:"Desconectado"})[status]||"No conectado";}
...
function renderChannelConnections(){var root=document.getElementById("channelConnectionCards"),payload=state.channelConnections;if(!root||!payload)return;var canManage=SERVER_ROLE==="admin",available=payload.meta_authorization_available||{};root.innerHTML=(payload.channels||[]).map(function(item){var channel=item.channel||item.id,status=item.status||"not_connected",soon=item.coming_soon||item.available===false,account=item.account_label?'<div class="channelAccount">'+esc(item.account_label)+'</div>':"",primary=channel==="whatsapp",actions='<span class="channelState '+attr(status)+'">'+esc(soon?"Próximamente":channelConnectionStatusLabel(status))+'</span>';if(!soon&&canManage){if(item.requires_selection){var options=(item.pending_assets||[]).map(function(asset){return '<option value="'+attr(asset.id)+'">'+esc(asset.label+(asset.detail?" · "+asset.detail:""))+'</option>';}).join("");actions+='<select class="channelAssetSelect" id="channelAsset-'+attr(channel)+'" aria-label="Selecciona la cuenta">'+options+'</select><button class="primaryBtn" type="button" data-channel="'+attr(channel)+'" onclick="selectChannelAsset(this.dataset.channel)">Confirmar cuenta</button>';}else if(item.connect_available){actions+='<button class="'+(primary?"primaryBtn":"ghostBtn")+'" type="button" data-channel="'+attr(channel)+'" onclick="connectChannel(this.dataset.channel)">'+(available[channel]===false?"Activación Meta pendiente":"Conectar con Meta")+'</button>';}else if(item.reconnect_available){actions+='<button class="'+(primary?"primaryBtn":"ghostBtn")+'" type="button" data-channel="'+attr(channel)+'" onclick="connectChannel(this.dataset.channel)">Reconectar</button>';}if(item.disconnect_available){actions+='<button class="ghostBtn" type="button" data-channel="'+attr(channel)+'" data-name="'+attr(item.name||channel)+'" onclick="disconnectChannel(this.dataset.channel,this.dataset.name)">Desconectar</button>';}}return '<article class="channelConnectCard'+(soon?" comingSoon":"")+(primary?" primaryChannel":"")+'"><span class="channelConnectIcon '+attr(channel)+'">'+esc(channelConnectionInitial(channel))+'</span><div class="channelConnectCopy"><h4>'+esc(item.name||channel)+'</h4><p>'+esc(item.description||"")+'</p>'+account+'</div><div class="channelConnectActions">'+actions+'</div></article>';}).join("");}
function loadChannelConnections(force){if(!PANEL_CHANNEL_CONNECTIONS_ENABLED||state.channelConnectionsLoading||(!force&&state.channelConnections))return;state.channelConnectionsLoading=true;api("/admin/panel/channel-connections",{redirectOnAuth:true}).then(function(body){state.channelConnections=body;renderChannelConnections();try{var url=new URL(location.href),result=url.searchParams.get("connection");if(result==="success")setChannelConnectionMessage("Conectado correctamente.","success");else if(result==="select")setChannelConnectionMessage("Elige la cuenta correcta para terminar la conexión.");else if(result==="error")setChannelConnectionMessage("No pudimos terminar la conexión. Intenta de nuevo o contacta a NextforIA.","error");url.searchParams.delete("connection");history.replaceState(null,"",url.pathname+url.search+url.hash);}catch(e){}}).catch(function(){setChannelConnectionMessage("No pudimos cargar tus canales. Intenta de nuevo.","error");}).finally(function(){state.channelConnectionsLoading=false;});}
function connectChannel(channel){setChannelConnectionMessage("Abriendo Meta para conectar el canal…");api("/admin/panel/channel-connections/"+encodeURIComponent(channel)+"/connect",{method:"POST",body:"{}"}).then(function(body){if(!body.authorization_url)throw new Error("authorization_unavailable");location.assign(body.authorization_url);}).catch(function(error){setChannelConnectionMessage(error.body&&error.body.message||"No pudimos terminar la conexión. Intenta de nuevo o contacta a NextforIA.","error");loadChannelConnections(true);});}
function selectChannelAsset(channel){var select=document.getElementById("channelAsset-"+channel),assetId=select&&select.value;if(!assetId)return;setChannelConnectionMessage("Verificando la cuenta elegida…");api("/admin/panel/channel-connections/"+encodeURIComponent(channel)+"/select",{method:"POST",body:JSON.stringify({asset_id:assetId})}).then(function(){state.channelConnections=null;setChannelConnectionMessage("Conectado correctamente.","success");loadChannelConnections(true);}).catch(function(){setChannelConnectionMessage("No pudimos terminar la conexión. Intenta de nuevo o contacta a NextforIA.","error");});}
function disconnectChannel(channel,name){if(!confirm("¿Desconectar "+name+"? NextforIA dejará de recibir nuevos mensajes de este canal."))return;setChannelConnectionMessage("Desconectando el canal…");api("/admin/panel/channel-connections/"+encodeURIComponent(channel)+"/disconnect",{method:"POST",body:"{}"}).then(function(){state.channelConnections=null;setChannelConnectionMessage("Canal desconectado.","success");loadChannelConnections(true);}).catch(function(error){setChannelConnectionMessage(error.body&&error.body.message||"No pudimos desconectar el canal. Contacta a NextforIA.","error");});}
function showTab(name){
  ...
  var pageTitle=name==="summary"?"Resumen":name==="tests"?"Pruebas":name==="plan"?"Mi plan":name==="channels"?"Conecta WhatsApp":name==="setup"?"Configuración de tu Nextfor IA":name==="retargeting"?"Seguimientos comerciales":name==="appointments"?"Citas":"Conversaciones";
  var pageSubtitle=name==="summary"?("Resultados de "+(PANEL_CONTEXT.v2?PANEL_CONTEXT.assignedBotName:"tu bot de atención")+" · Últimos 7 días"):name==="tests"?"Herramientas seguras para validar el bot.":name==="plan"?"Plan, módulos y consumo":name==="channels"?"Empieza por el canal donde más te escriben":name==="setup"?"Tu negocio, tu voz y tus reglas en un solo lugar":name==="retargeting"?"Cola segura, aprobaciones, cancelaciones y auditoría":name==="appointments"?"Tu agenda llenándose, sin perseguir confirmaciones.":"La IA atiende y te deja solo lo que necesita de ti.";
...
function planPriceLine(plan){var setup=panelMoney(plan.precio_setup),mensual=panelMoney(plan.precio_mensual);var head=setup?(setup+" setup"):"Inversión inicial por definir";var sub=(mensual?(mensual+"/mes"):"Mensualidad por definir")+" · "+panelChats(plan.chats_incluidos);return '<div class="priceLine"><strong>'+esc(head)+'</strong><span>'+esc(sub)+'</span></div>';}
...
function renderOnboardingSummary(payload){var box=document.getElementById("setupConfigSummary"),details=document.getElementById("onboardingQuestionnaireList");if(!box)return;var onboarding=payload&&payload.onboarding||{},answers=onboarding.answers||{},appt=answers.appointment_setup||{},svc=answers.customer_service_setup||{},goal=answers.setup_goal,business=setupPathGet(answers,"business.brand_name")||appt.business_name||PANEL_CONTEXT.businessName,clients=setupPathGet(answers,"operations.monthly_customer_volume"),tone=svc.tone||appt.assistant_tone||setupPathGet(answers,"voice.formality"),whatsapp=setupPathGet(answers,"meta.whatsapp_number"),instagram=appt.instagram_username||setupPathGet(answers,"meta.instagram_account"),email=appt.channel_email||setupPathGet(answers,"business.contact_email"),other=appt.other_channels,channels=[whatsapp?("WhatsApp "+whatsapp):"",instagram?("Instagram "+instagram):"",email?("Correo "+email):"",other].filter(Boolean).join(" · "),updated=onboarding.last_updated_at||onboarding.updated_at||"",status=onboarding.setup_completed?"Cuestionario completo":"En progreso";var cards=[["Empresa",business,"Marca y cuenta asociada al tenant."],["Bot contratado",setupGoalLabel(goal)||PANEL_CONTEXT.assignedBotName,"Plan: "+(PANEL_CONTEXT.planName||"Asignado")],["Clientes al mes",clients,"Aproximado para dimensionar plan y consumo."],["Cómo debe hablar",tone,"Voz base para responder como tu equipo."],["Canales recopilados",channels,"Datos guardados; conexión técnica pendiente."],["Estado",status,updated?("Última actualización: "+when(updated)):"Se actualizará al guardar."]];box.innerHTML=cards.map(function(item){return '<article class="setupConfigCard"><small>'+esc(item[0])+'</small><strong>'+esc(setupShort(item[1]))+'</strong><p>'+esc(item[2])+'</p></article>';}).join("");text("channelConnectionSummary",PANEL_CHANNEL_CONNECTIONS_ENABLED?"Conecta WhatsApp con la autorización oficial de Meta. Instagram queda como opción adicional y puedes continuar más tarde.":"La conexión de canales todavía no está disponible para esta cuenta.");if(details){var questions=payload&&payload.questionnaire&&payload.questionnaire.questions||[],rows=[];questions.filter(function(question){return question&&question.active!==false&&question.path;}).sort(function(a,b){return(Number(a.order)||0)-(Number(b.order)||0);}).forEach(function(question){var value=setupPathGet(answers,question.path);var readable=readableSetupValue(value);if(!readable||readable==="unknown")return;rows.push([question.section||"general",question.label||question.path,readable]);});["meta.whatsapp_number","appointment_setup.instagram_username","appointment_setup.channel_email","appointment_setup.other_channels"].forEach(function(path){var exists=rows.some(function(row){return row[1]===path;}),value=setupPathGet(answers,path);if(!exists&&readableSetupValue(value))rows.push(["canales",path,readableSetupValue(value)]);});details.innerHTML=rows.length?rows.map(function(row){return '<article class="questionnaireItem"><small>'+esc(row[0])+'</small><strong>'+esc(row[1])+'</strong><p>'+esc(setupShort(row[2]))+'</p></article>';}).join(""):'<article class="questionnaireItem"><small>Cuestionario</small><strong>Aún no hay respuestas guardadas.</strong></article>';}}
