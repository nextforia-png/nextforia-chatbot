...
function renderAdminChannels(rows){var root=document.getElementById("adminChannelRows");if(!root)return;root.textContent="";if(!rows||!rows.length){root.appendChild(el("div","invite-loading","Todavía no hay tenants con canales disponibles."));return;}rows.forEach(function(row){var line=el("article","channel-admin-row"),client=el("div");client.appendChild(el("strong",null,row.company_name||row.tenant_id));client.appendChild(el("small",null,row.tenant_id));var channel=el("div");channel.appendChild(el("strong",null,adminChannelName(row.channel)));var status=adminChannelStatus(row.status);channel.appendChild(el("span","badge "+status[1]+" dot",status[0]));var account=el("div");account.appendChild(el("strong",null,row.account_label||"Sin cuenta conectada"));account.appendChild(el("small",null,row.account_id||"—"));var detail=el("div");detail.appendChild(el("strong",null,adminChannelDate(row.last_verified_at)));detail.appendChild(el("small",null,"Conectó: "+(row.connected_by||"—")+" · Desconectó: "+(row.disconnected_by||"—")));if(row.last_error)detail.appendChild(el("small","channel-admin-error","Último error: "+row.last_error));var actions=el("div","channel-admin-actions");if(!row.protected_legacy&&["connected","needs_attention"].includes(row.status)){var verify=el("button","button","Verificar");verify.type="button";verify.addEventListener("click",function(){adminChannelAction(row,"verify",false);});actions.appendChild(verify);}if(!row.protected_legacy&&["connected","needs_attention","disconnected","not_connected"].includes(row.status)){var reconnect=el("button","button",row.status==="connected"?"Renovar acceso":"Ayudar a reconectar");reconnect.type="button";reconnect.addEventListener("click",function(){startSetupChannelConnection(row.tenant_id,row.channel);});actions.appendChild(reconnect);}if(!row.protected_legacy&&["connected","needs_attention","connecting"].includes(row.status)){var disconnect=el("button","button danger","Desconectar");disconnect.type="button";disconnect.addEventListener("click",function(){adminChannelAction(row,"disconnect",true);});actions.appendChild(disconnect);}if(row.protected_legacy)actions.appendChild(el("span","badge neutral","Conexión RAV protegida"));line.append(client,channel,account,detail,actions);root.appendChild(line);});}
