...
function renderBusinessContext(business){if(!PANEL_CONTEXT.v2||!business)return;var name=panelBusinessName(business),initials=panelInitials(name),logo=business.logo_data_url||"";PANEL_CONTEXT.businessName=name;text("brandName",name);applyPanelLogo(logo,name);var mobileName=document.querySelector(".mobileBrand h1"),mobileAvatar=document.querySelector(".mobileAvatar");if(mobileName)mobileName.textContent=name;if(mobileAvatar)mobileAvatar.textContent=initials;document.title="Panel de control · "+name;}
