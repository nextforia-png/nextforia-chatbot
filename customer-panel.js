...
.notificationEmailSettings{display:grid;gap:14px;border:1px solid var(--line);background:#fff;border-radius:18px;padding:18px;color:var(--navy-900);box-shadow:var(--shadow-soft)}.notificationEmailSettings[hidden]{display:none}.notificationEmailHead{display:flex;align-items:center;justify-content:space-between;gap:16px}.notificationEmailHead strong{display:block;font-size:14px}.notificationEmailHead span{display:block;margin-top:3px;color:var(--slate-500);font-size:12px;font-weight:700}.notificationEmailSwitch{display:inline-flex;align-items:center;gap:8px;font-size:12px;font-weight:900;white-space:nowrap}.notificationEmailSwitch input{width:18px;height:18px;accent-color:var(--cyan-500)}.notificationEmailRecipient{padding:11px 13px;border:1px solid #DDE6F1;border-radius:12px;background:#F8FAFD;font-size:12px;color:var(--slate-500)}.notificationEmailRecipient strong{color:var(--navy-900)}.notificationEmailTypes{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:9px}.notificationEmailType{display:flex;align-items:flex-start;gap:9px;padding:10px 11px;border:1px solid #E3E9F2;border-radius:12px;background:#FBFCFE;font-size:12px;font-weight:800;line-height:1.35}.notificationEmailType input{margin-top:1px;accent-color:var(--cyan-500)}.notificationEmailActions{display:flex;align-items:center;justify-content:space-between;gap:12px}.notificationEmailStatus{font-size:12px;color:var(--slate-500);font-weight:700}
...
@media(max-width:760px){
  ...
  .notificationsView,#panel-notifications,.notificationEmailSettings,.notificationEmailHead>div,.notificationEmailActions,.notificationEmailStatus{min-width:0;max-width:100%}.notificationControls{align-items:flex-start;flex-direction:column}.notificationControls .primaryBtn{width:100%}.notificationEmailSettings{overflow:hidden;padding:15px}.notificationEmailHead,.notificationEmailActions{align-items:flex-start;flex-direction:column}.notificationEmailHead>div,.notificationEmailSwitch,.notificationEmailRecipient,.notificationEmailActions{width:100%}.notificationEmailHead strong,.notificationEmailHead span,.notificationEmailRecipient,.notificationEmailRecipient strong,.notificationEmailStatus{overflow-wrap:anywhere}.notificationEmailSwitch{justify-content:flex-start;white-space:normal}.notificationEmailTypes{width:100%;grid-template-columns:minmax(0,1fr)}.notificationEmailType{width:100%;min-width:0;box-sizing:border-box}.notificationEmailType input{flex:0 0 auto}.notificationEmailType span{min-width:0;overflow-wrap:anywhere}.notificationEmailActions .primaryBtn{width:100%}.handoffToast{right:16px;top:16px;grid-template-columns:42px 1fr}.handoffToast button{grid-column:1/-1;width:100%}
      ...
      <section class="${viewClass('panel-notifications')}" id="panel-notifications">
        <div class="notificationsView">
          ...
          <section class="notificationEmailSettings" id="notificationEmailSettings" hidden>
            <div class="notificationEmailHead"><div><strong>Recibe también los avisos por correo</strong><span>Se enviarán únicamente al correo de tu acceso oficial a esta empresa.</span></div><label class="notificationEmailSwitch"><input id="notificationEmailEnabled" type="checkbox"> Correos activos</label></div>
            <div class="notificationEmailRecipient">Destinatario: <strong id="notificationEmailRecipient">—</strong></div>
            <div class="notificationEmailTypes">
              <label class="notificationEmailType"><input type="checkbox" data-notification-email-type="payment_pending"><span>Pedidos y pagos por confirmar</span></label>
              <label class="notificationEmailType"><input type="checkbox" data-notification-email-type="shipping_pending"><span>Pedidos pendientes de envío</span></label>
              <label class="notificationEmailType"><input type="checkbox" data-notification-email-type="sales_opportunity"><span>Oportunidades de venta</span></label>
              <label class="notificationEmailType"><input type="checkbox" data-notification-email-type="product_update"><span>Novedades de Nextfor</span></label>
              <label class="notificationEmailType"><input type="checkbox" data-notification-email-type="human_attention"><span>Conversaciones que requieren atención</span></label>
            </div>
            <div class="notificationEmailActions"><span class="notificationEmailStatus" id="notificationEmailStatus">Elige qué avisos quieres recibir.</span><button class="primaryBtn" id="notificationEmailSave" type="button" onclick="saveNotificationEmailPreferences()">Guardar correos</button></div>
          </section>
...
function renderNextforNotifications(){var root=document.getElementById("nextforNotifications"),notifications=state.notifications||{},items=notifications.items||[],pending=Number(notifications.pending_count)||0,unread=Number(notifications.unread_count)||0;["navNotificationCount","mnavNotificationCount","mobileHeaderNotificationCount"].forEach(function(id){var el=document.getElementById(id);if(el){el.textContent=unread||"";el.style.display=unread?"grid":"none";}});if(!root)return;if(!items.length){root.innerHTML='<article class="notificationEmpty">No tienes notificaciones pendientes. Los nuevos pedidos y las solicitudes de ayuda aparecerán aquí inmediatamente.</article>';return;}root.innerHTML=items.map(function(item){var questions=(item.pending_questions||[]).slice(0,6).map(function(question){return '<span>'+esc(question.label||question.path)+'</span>';}).join(""),actionUrl=String(item.action_url||""),itemId=String(item.id||""),action=actionUrl?'<button class="primaryBtn" type="button" data-action-url="'+attr(actionUrl)+'" data-notification-id="'+attr(itemId)+'" onclick="event.stopPropagation();notificationAction(this.dataset.actionUrl,this.dataset.notificationId)">'+esc(item.action_label||"Abrir")+'</button>':'',high=item.priority==="high",liveType=["human_handoff_required","customer_order_created"].includes(item.type),unreadClass=liveType&&!item.read?" unread":"",clickAttrs=actionUrl?' clickable" role="button" tabindex="0" data-action-url="'+attr(actionUrl)+'" data-notification-id="'+attr(itemId)+'" onclick="notificationAction(this.dataset.actionUrl,this.dataset.notificationId)" onkeydown="notificationCardKey(event,this)':'"',icon=item.type==="human_handoff_required"?"🙋":(item.type==="customer_order_created"?"📦":(high?PANEL_CHECK_ICON:"✧"));return '<article class="notificationCard '+(high?"high":"")+unreadClass+clickAttrs+'><span class="notificationIcon">'+icon+'</span><div class="notificationCopy"><h4>'+esc(item.title||"Notificación")+'</h4><p>'+esc(item.message||"")+'</p>'+(questions?'<div class="pendingQuestionList">'+questions+'</div>':"")+'</div><div>'+action+'</div></article>';}).join("");if(pending)renderPendingSetupReminder(pending);}
...
function renderNotificationEmailPreferences(){var root=document.getElementById("notificationEmailSettings"),preferences=state.notificationEmailPreferences,enabled=document.getElementById("notificationEmailEnabled"),save=document.getElementById("notificationEmailSave");if(!root)return;if(!preferences||preferences.available!==true){root.hidden=true;return;}root.hidden=false;text("notificationEmailRecipient",preferences.recipient||"—");if(enabled)enabled.checked=preferences.enabled===true;document.querySelectorAll("[data-notification-email-type]").forEach(function(input){input.checked=!preferences.types||preferences.types[input.dataset.notificationEmailType]!==false;});if(save)save.disabled=false;text("notificationEmailStatus",preferences.enabled?"Correos activos para esta cuenta.":"Los correos están apagados hasta que los actives.");}
function loadNotificationEmailPreferences(force){if(DEMO_MODE||state.notificationEmailLoading||(!force&&state.notificationEmailPreferences))return;state.notificationEmailLoading=true;api("/admin/panel/notifications/email-preferences",{redirectOnAuth:true}).then(function(result){state.notificationEmailPreferences=result.preferences||null;renderNotificationEmailPreferences();}).catch(function(error){var root=document.getElementById("notificationEmailSettings");if(root)root.hidden=true;if(error&&error.status!==403)text("notificationEmailStatus","No pudimos cargar la configuración de correos.");}).finally(function(){state.notificationEmailLoading=false;});}
function saveNotificationEmailPreferences(){var button=document.getElementById("notificationEmailSave"),enabled=document.getElementById("notificationEmailEnabled"),types={};document.querySelectorAll("[data-notification-email-type]").forEach(function(input){types[input.dataset.notificationEmailType]=!!input.checked;});if(button){button.disabled=true;button.textContent="Guardando…";}text("notificationEmailStatus","Guardando sin cambiar los datos de tu empresa…");api("/admin/panel/notifications/email-preferences",{method:"PUT",body:JSON.stringify({enabled:!!(enabled&&enabled.checked),types:types})}).then(function(result){state.notificationEmailPreferences=result.preferences;renderNotificationEmailPreferences();text("notificationEmailStatus",result.preferences&&result.preferences.enabled?"✓ Correos activados para "+result.preferences.recipient:"✓ Preferencias guardadas. Los correos están apagados.");}).catch(function(error){text("notificationEmailStatus",error&&error.status===503?"El correo todavía no está habilitado en este ambiente.":"No pudimos guardar. Intenta nuevamente.");}).finally(function(){if(button){button.disabled=false;button.textContent="Guardar correos";}});}
