...
app.get("/admin/nextfor-appointment-bot", (req, res) => res.type("html").send(renderGoogleOAuthHomepage()));
app.get("/privacy", (req, res) => res.type("html").send(renderPrivacyPolicy()));
app.get("/admin/privacy", (req, res) => res.type("html").send(renderPrivacyPolicy()));
...
const BOT_VERSION = "v398-customer-panel-conversation-visibility";  // bump cada release; usado por endpoints /admin/*
...
app.use("/admin", async function revalidateCustomerSession(req, res, next) {
  ...
  try {
    ...
    if (!req.dashboardVerifiedSession) clearDashboardSessionCookie(req, res);
    return next();
  } catch (_) {
...
app.get("/admin/panel/notifications/events", async (req, res) => {
  ...
  // Subscribe before the initial snapshot. Otherwise a notification created
  // between the snapshot and events.on() is invisible until the polling pass.
