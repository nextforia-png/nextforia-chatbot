...
function completeWhatsAppEmbeddedSignup(){
  ...
  api("/admin/panel/channel-connections/whatsapp/complete",{method:"POST",body:JSON.stringify({state:pending.config.oauth_state,code:pending.code,session:pending.session})}).then(function(body){
    state.whatsappEmbedded=null;state.channelConnections=null;state.whatsappActivationPolls=0;if(body.connection&&body.connection.status==="connecting")setChannelConnectionMessage("Meta aceptó el número. Nextfor está terminando y verificando la conexión automáticamente.","success");else setChannelConnectionMessage("Listo. WhatsApp quedó conectado a tu Nextfor.","success");loadChannelConnections(true);
  }).catch(function(error){
...
function launchWhatsAppEmbeddedSignup(config){
  ...
  loadMetaSdk(config).then(function(FB){
    FB.login(function(response){
      ...
      extras:{version:"v3",setup:{},featureType:"whatsapp_business_app_onboarding"}
    });
  }).catch(function(){
...
window.addEventListener("message",function(event){
  ...
  if(payload.event==="FINISH"||payload.event==="FINISH_WHATSAPP_BUSINESS_APP_ONBOARDING"){
    ...
    state.whatsappEmbedded.session={waba_id:String(session.waba_id),phone_number_id:session.phone_number_id?String(session.phone_number_id):"",business_id:session.business_id?String(session.business_id):"",coexistence:payload.event==="FINISH_WHATSAPP_BUSINESS_APP_ONBOARDING",onboarding_event:payload.event};
...
function scheduleWhatsAppActivationCheck(){if(state.whatsappActivationTimer||state.whatsappActivating||(state.whatsappActivationPolls||0)>=12)return;state.whatsappActivationTimer=setTimeout(function(){state.whatsappActivationTimer=null;activateWhatsApp(null,true);},5000);}
function activateWhatsApp(button,automatic){if(state.whatsappActivating)return;var input=document.getElementById("whatsappRegistrationPin"),pin=String(input&&input.value||"").trim(),needsPin=!!input;if(needsPin&&!/^\d{6}$/.test(pin)){setChannelConnectionMessage("Crea un PIN de seguridad de seis dígitos para WhatsApp.","error");input.focus();return;}if(DEMO_MODE){if(!automatic)setChannelConnectionMessage(needsPin?"Demo: WhatsApp quedaría activo con un PIN válido.":"Demo: Nextfor verificaría la conexión automáticamente.","success");return;}if(!automatic)state.whatsappActivationPolls=0;state.whatsappActivating=true;if(button){button.disabled=true;button.textContent=needsPin?"Activando…":"Revisando…";}var requestBody=needsPin?JSON.stringify({pin:pin}):"{}";if(input)input.value="";pin="";if(!automatic)setChannelConnectionMessage(needsPin?"Guardando el PIN de seguridad y activando WhatsApp Cloud API…":"Verificando la conexión de WhatsApp con Meta…");api("/admin/panel/channel-connections/whatsapp/activate",{method:"POST",body:requestBody}).then(function(body){state.channelConnections=null;if(body.connection&&body.connection.status==="connected"){state.whatsappActivationPolls=12;if(state.whatsappActivationTimer){clearTimeout(state.whatsappActivationTimer);state.whatsappActivationTimer=null;}setChannelConnectionMessage("Listo. WhatsApp quedó conectado y preparado para recibir mensajes.","success");}else{state.whatsappActivationPolls=(state.whatsappActivationPolls||0)+1;if(!automatic)setChannelConnectionMessage(body.connection&&body.connection.registration_pin_required?"Meta verificó el número. Crea el PIN aquí para terminar la activación.":"Meta todavía está terminando la conexión. Nextfor seguirá verificando automáticamente.","success");}loadChannelConnections(true);}).catch(function(error){state.channelConnections=null;state.whatsappActivationPolls=(state.whatsappActivationPolls||0)+1;if(!automatic)setChannelConnectionMessage(error.body&&error.body.message||"No pudimos verificar la conexión. Puedes volver a autorizarla desde este panel.","error");loadChannelConnections(true);}).finally(function(){requestBody="";state.whatsappActivating=false;if(button&&button.isConnected){button.disabled=false;button.textContent=needsPin?"Guardar PIN y activar":"Revisar conexión";}if(!needsPin)scheduleWhatsAppActivationCheck();});}
