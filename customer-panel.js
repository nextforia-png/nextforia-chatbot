...
@media(max-width:760px){
  ...
  .customer-panel-mobile-v389.mobile-detail-view .mobileDetailHeader{position:sticky;top:0;z-index:35;min-height:68px;display:flex;align-items:center;gap:11px;padding:10px 13px;border-bottom:1px solid #DEE6F0;background:rgba(255,255,255,.97);backdrop-filter:blur(12px)}
  .customer-panel-mobile-v389.mobile-detail-view .mobileDetailHeader>button{width:44px;height:44px;display:grid;place-items:center;border:1px solid #DEE6F0;border-radius:13px;background:#F7F9FC;color:#0A1836;font-size:20px}
  .customer-panel-mobile-v389.mobile-detail-view .mobileDetailHeader>div{min-width:0;display:grid;gap:1px}
  .customer-panel-mobile-v389.mobile-detail-view .mobileDetailHeader strong{font-family:var(--font-display);font-size:18px}
  .customer-panel-mobile-v389.mobile-detail-view .mobileDetailHeader span{overflow:hidden;color:#74839D;font-size:10px;text-overflow:ellipsis;white-space:nowrap}
...
function mobileDetailBack(){var profileTabs=["plan","channels","setup","notifications","tests"];if(profileTabs.includes(state.tab)){openProfile();return;}if(state.bot==="appointments"){showTab("appointments");if(typeof showAppointmentSection==="function")showAppointmentSection("agenda");return;}showTab("summary");}
function updateMobileDetailHeader(){var header=document.getElementById("mobileDetailHeader"),title=document.getElementById("mobileDetailTitle"),subtitle=document.getElementById("mobileDetailSubtitle"),labels={orders:["Pedidos","Confirma y despacha sin salir de Nextfor"],retargeting:["Oportunidades","Ventas que vale la pena retomar"],plan:["Mi plan","Tu servicio y posibilidades de crecimiento"],channels:["Conectar canales","Dónde atenderá tu Nextfor"],setup:["Configuración","Afina cómo trabaja tu bot"],notifications:["Notificaciones","Avisos que necesitan tu atención"],tests:["Pruebas","Valida tu bot de forma segura"]},copy=labels[state.tab],detail=!!copy;if(header)header.hidden=!detail;if(title)title.textContent=detail?copy[0]:"";if(subtitle)subtitle.textContent=detail?copy[1]:"";document.body.classList.toggle("mobile-detail-view",detail);}
...
function updateDemoOrderAction(order,action,tracking,trackingUrl){if(action==="confirm_payment")order.stage="pagado";else if(action==="start_preparation")order.stage="preparacion";else if(action==="mark_sent")order.stage="enviado";else if(action==="cancel")order.stage="cancelado";else if(action==="send_tracking"||action==="save_tracking_draft"){order.tracking_number=tracking;order.guide=tracking;order.tracking_url=trackingUrl;order.tracking_sent_at=action==="send_tracking"?new Date().toISOString():"";}renderOrders();panelToast("Pedido demo actualizado. No se modificó información real.");}
function orderAction(action){var order=state.orders.find(function(item){return orderKey(item)===String(state.selectedOrder);});if(!order||!SERVER_CAPABILITIES.manage_orders)return;var tracking=document.getElementById("orderTrackingInput"),trackingUrl=document.getElementById("orderTrackingUrlInput"),trackingValue=tracking?tracking.value.trim():"",trackingUrlValue=trackingUrl?trackingUrl.value.trim():"",trackingAction=action==="send_tracking"||action==="save_tracking_draft";if(trackingAction&&!trackingValue){panelToast("Escribe el número de guía antes de continuar.",true);if(tracking)tracking.focus();return;}if(trackingAction&&!trackingUrlValue){panelToast("Agrega el enlace de rastreo de la transportadora.",true);if(trackingUrl)trackingUrl.focus();return;}if(trackingAction&&trackingUrlValue.toLowerCase().indexOf("https://")!==0){panelToast("El enlace de rastreo debe comenzar por https://.",true);if(trackingUrl)trackingUrl.focus();return;}if(DEMO_MODE){updateDemoOrderAction(order,action,trackingValue,trackingUrlValue);return;}document.querySelectorAll("[data-order-action]").forEach(function(button){button.disabled=true;});api("/admin/panel/orders/action",{method:"POST",body:JSON.stringify({order_id:order.id,action:action,tracking_number:trackingValue,tracking_url:trackingUrlValue})}).then(function(result){var index=state.orders.findIndex(function(item){return orderKey(item)===orderKey(result.order);});if(index>=0)state.orders[index]=result.order;panelToast(action==="send_tracking"?"Guía y enlace entregados al cliente y guardados.":action==="save_tracking_draft"?"Guía y enlace guardados. Se enviarán después de confirmar el pago.":"Pedido actualizado.");renderOrders();}).catch(function(error){panelToast((error.body&&error.body.message)||"No pudimos actualizar el pedido.",true);renderOrders();});}
...
var renderOrdersBase=renderOrders;
renderOrders=function(){renderOrdersBase();document.querySelectorAll(".orderRow").forEach(function(row){var order=state.orders.find(function(item){return orderKey(item)===String(row.dataset.order);}),value=row.querySelector("b");if(order&&value&&orderShippingPending(order))value.textContent=orderTotalLabel(order);});var selected=state.orders.find(function(order){return orderKey(order)===String(state.selectedOrder);}),canManage=!!SERVER_CAPABILITIES.manage_orders,canSend=selected&&["pagado","preparacion","enviado"].includes(selected.stage),trackingButton=document.querySelector("[data-action=send_tracking]");document.querySelectorAll("#orderTrackingInput,#orderTrackingUrlInput").forEach(function(control){control.disabled=!canManage;});if(trackingButton){trackingButton.dataset.action=canSend?"send_tracking":"save_tracking_draft";trackingButton.disabled=!canManage;trackingButton.textContent=canSend?(selected.tracking_sent_at?"Actualizar y enviar":"Enviar guía y enlace"):(selected&&selected.tracking_number&&selected.tracking_url?"Actualizar datos":"Guardar guía y enlace");}var trackingCard=document.querySelector(".orderTrackingCard");if(trackingCard&&selected&&!selected.tracking_sent_at){var trackingTitle=trackingCard.querySelector("strong"),trackingCopy=trackingCard.querySelector("p");if(trackingTitle)trackingTitle.textContent=selected.tracking_number&&selected.tracking_url?(canSend?"Guía y enlace listos para enviar":"Guía y enlace guardados"):"Agrega la guía y el enlace de seguimiento";if(trackingCopy)trackingCopy.textContent=canSend?"Se enviarán por el mismo canal donde llegó el pedido.":"Puedes guardarlos ahora. Nextfor no los enviará hasta confirmar el pago.";}if(!selected||!orderShippingPending(selected))return;var productRows=document.querySelectorAll("#orderDetailPane .orderProducts>div");productRows.forEach(function(row){var label=row.querySelector("span"),value=row.querySelector("strong");if(!label||!value)return;if(label.textContent==="Envío")value.textContent=orderShippingLabel(selected);if(row.classList.contains("orderTotal")){label.textContent=orderTotalHeading(selected);value.textContent=orderTotalLabel(selected);}});};
