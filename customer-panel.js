...
function showTab(name){
  ...
  if(name==="orders")loadOrders(false);if(name==="channels")loadChannelConnections(false);if(name==="setup"){loadBotSetup();loadBotPersonality(false);}if(name==="notifications")renderNextforNotifications();if(name==="retargeting")loadRetargeting(false);if(name==="appointments")loadAppointments();renderInbox();renderPlan();updateMobilePrimaryNav();window.scrollTo(0,0);
}
...
function orderChannelGlyph(channel){return channel==="whatsapp"?"◉":channel==="instagram"?"◎":channel==="messenger"?"●":"✉";}
