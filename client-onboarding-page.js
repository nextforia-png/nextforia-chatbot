...
module.exports = function renderClientOnboarding(res, options) {
  ...
  const planDescription = plan && plan.descripcion || "Tu plan contratado se administra desde Nextfor IA.";
...
<main class="setupPage hidden" id="setupPage">
  ...
  <div class="layout">
    <section class="formCard">
      ...
      <form id="setupForm" novalidate>
        ...
        <section class="panel" data-step="3">
          <h2>Revisa lo que contrataste</h2><p>El plan y el bot vienen directamente de Super Admin. Puedes consultar esta información, pero no cambiar precios ni módulos desde aquí.</p>
          <div class="contractGrid">
            <article class="contractItem"><small>Plan contratado</small><strong>${escapeHtml(planName)}</strong><p>${escapeHtml(planDescription)}</p></article>
          ...
          <p class="locked">Los cambios comerciales requieren validación de Nextfor IA.</p>
    ...
    <aside class="right">
      ...
      <section class="contractCard"><div class="contractTitle"><img src="/admin/assets/lumen.png" alt=""><div><strong>Tu cuenta ya tiene</strong><span>Información asignada desde Super Admin</span></div></div><div class="contractGrid"><div class="contractItem"><small>Plan</small><strong>${escapeHtml(planName)}</strong></div><div class="contractItem"><small>Bot</small><strong>${escapeHtml(botName)}</strong></div></div><div class="locked">Información comercial de solo lectura.</div></section>
    </aside>
  </div>
</main>
...
function completion(answers){var all=Object.keys(REQUIRED).reduce(function(list,key){return list.concat(REQUIRED[key])},[]),done=all.filter(function(path){return String(getPath(answers,path)||"").trim()}).length;return Math.round(done/all.length*100)}
...
function request(status){var answers=collect();if(DEMO){INITIAL.answers=answers;INITIAL.status=status;INITIAL.completion=completion(answers);INITIAL.setup_completed=status==="completed";return Promise.resolve({onboarding:INITIAL,redirect:"/admin/panel?tab=summary"})}return fetch(API_PATH,{method:"PUT",headers:{"content-type":"application/json"},body:JSON.stringify({answers:answers,status:status})}).then(function(response){return response.json().then(function(body){if(!response.ok){var error=new Error(body.message||body.error||"No se pudo guardar");error.body=body;throw error}return body})})}
