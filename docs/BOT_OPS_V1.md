...
2. The daily review advances a durable event cursor and reads only new events plus already-flagged open handoffs.
...
6. Super Admin reads `/admin/bot-ops/summary`. Critical daily findings are also sent by email through Resend without customer message bodies or identifiers.
