...
module.exports = function renderCustomerPanel(res, options) {
  ...
  const PAGE_TITLES = { summary: "Resumen", conversations: "Conversaciones", appointments: "Citas", plan: "Mi plan", channels: "Conecta tus canales", setup: "Configuración de tu Nextfor IA", retargeting: "Seguimientos comerciales", tests: "Pruebas" };
  const PAGE_SUBTITLES = { summary: summarySubtitle, conversations: "La IA atiende y te deja solo lo que necesita de ti.", appointments: "Tu agenda llenándose, sin perseguir confirmaciones.", plan: "Plan, módulos y consumo", channels: "Elige dónde quieres atender a tus clientes", setup: "Tu negocio, tu voz y tus reglas en un solo lugar", retargeting: "Cola segura, aprobaciones, cancelaciones y auditoría", tests: "Herramientas seguras para validar el bot." };
      ...
      ${channelConnectionsV1Enabled ? `<section class="${viewClass('panel-channels')}" id="panel-channels">
        <div class="channelsView">
          <section class="channelsHero">
            ...
            <p>Elige uno o varios canales. Meta te pedirá autorización en su sitio oficial; NextforIA nunca te pedirá tokens, claves ni credenciales técnicas.</p>
          </section>
        ...
        <div class="setupView">
          <section class="setupSummaryPanel">
            ...
            <div class="setupFlowSteps">
              ...
              <article class="setupFlowStep next"><b>3</b><strong>Conectar canales</strong><span>WhatsApp, Instagram u otros se conectan después.</span></article>
            ...
            <div class="channelPlan"><div><h4>Próximo paso: conectar canales y bot</h4><p id="channelConnectionSummary">${channelConnectionsV1Enabled ? "Conecta WhatsApp, Instagram o Messenger con la autorización oficial de Meta. Puedes elegir uno, varios o continuar más tarde." : "La conexión de canales todavía no está disponible para esta cuenta."}</p></div>${channelConnectionsV1Enabled ? '<button class="primaryBtn" type="button" onclick="showTab(\'channels\')">Conectar canales</button>' : '<button class="primaryBtn" type="button" disabled>Conexión pendiente</button>'}</div>
...
function showTab(name){
  ...
  var pageTitle=name==="summary"?"Resumen":name==="tests"?"Pruebas":name==="plan"?"Mi plan":name==="channels"?"Conecta tus canales":name==="setup"?"Configuración de tu Nextfor IA":name==="retargeting"?"Seguimientos comerciales":name==="appointments"?"Citas":"Conversaciones";
  var pageSubtitle=name==="summary"?("Resultados de "+(PANEL_CONTEXT.v2?PANEL_CONTEXT.assignedBotName:"tu bot de atención")+" · Últimos 7 días"):name==="tests"?"Herramientas seguras para validar el bot.":name==="plan"?"Plan, módulos y consumo":name==="channels"?"Elige dónde quieres atender a tus clientes":name==="setup"?"Tu negocio, tu voz y tus reglas en un solo lugar":name==="retargeting"?"Cola segura, aprobaciones, cancelaciones y auditoría":name==="appointments"?"Tu agenda llenándose, sin perseguir confirmaciones.":"La IA atiende y te deja solo lo que necesita de ti.";
