...
function renderSuperAdminPanel(res, options) {
  ...
  function renderLeadRows(rows) {
    ...
    return rows.map(function (row) {
      return '<div class="lead-row"><div><strong>' + escapeHtml(row.company_name || row.tenant_id) + '</strong><small>' + escapeHtml(row.tenant_id || "—") + '</small></div><div><strong>' + escapeHtml(row.admin_email || "—") + '</strong><small>' + escapeHtml(row.contact_phone || "Sin teléfono") + '</small></div><div><span class="badge ' + leadBadge(row.stage) + ' dot">' + escapeHtml(row.stage_label || "Lead") + '</span><small>' + num(row.completion || 0) + '% setup</small></div><div><strong>' + escapeHtml(row.plan_id || "Plan inicial") + '</strong><small>' + escapeHtml(row.assigned_bot_id || "Bot inicial") + '</small></div><div><small>' + escapeHtml(row.next_action || "Revisar lead.") + '</small></div></div>';
    }).join("");
  }
...
@media(max-width:980px){.setup-review-layout{grid-template-columns:1fr}.setup-review-fields{grid-template-columns:1fr}}
...
<section class="view" data-panel="leads"><div class="stack">
  ...
  <section class="card table-card"><div class="list-head"><h2>Leads activos</h2><span class="badge info" id="leadRowsCount">${leadsData && leadsData.rows ? num(leadsData.rows.length) : 0}</span></div><div class="table-scroll"><div class="invite-head" aria-hidden="true"><span>Empresa</span><span>Contacto</span><span>Etapa</span><span>Plan / bot</span><span>Próximo paso</span></div><div id="leadPipelineRows">${leadRows}</div></div></section>
...
<section class="view" data-panel="incidents"><div class="stack">
  <div class="callout" style="margin-top:0"><div><strong>${integrationLive ? "WhatsApp RAV en funcionamiento" : "Activacion del numero real pendiente"}</strong><p>${integrationLive ? "La integracion #1 de Nextfor opera con el numero real y conserva monitoreo tecnico." : "Meta ya aprobo la app. Falta conectar y verificar " + escapeHtml(integration.target_display_phone) + " antes de iniciar conversaciones reales."}</p></div><span class="badge ${integrationTone} dot">${integrationLive ? "Operativo" : "Abierto"}</span></div>
...
function renderAdminChannels(rows){var root=document.getElementById("adminChannelRows");if(!root)return;root.textContent="";if(!rows||!rows.length){root.appendChild(el("div","invite-loading","Todavía no hay tenants con canales disponibles."));return;}rows.forEach(function(row){var line=el("article","channel-admin-row"),client=el("div");client.appendChild(el("strong",null,row.company_name||row.tenant_id));client.appendChild(el("small",null,row.tenant_id));var channel=el("div");channel.appendChild(el("strong",null,adminChannelName(row.channel)));var status=adminChannelStatus(row.status);channel.appendChild(el("span","badge "+status[1]+" dot",status[0]));var account=el("div");account.appendChild(el("strong",null,row.account_label||"Sin cuenta conectada"));account.appendChild(el("small",null,row.account_id||"—"));var detail=el("div");detail.appendChild(el("strong",null,adminChannelDate(row.last_verified_at)));detail.appendChild(el("small",null,"Conectó: "+(row.connected_by||"—")+" · Desconectó: "+(row.disconnected_by||"—")));if(row.last_error)detail.appendChild(el("small","channel-admin-error","Último error: "+row.last_error));var actions=el("div","channel-admin-actions");if(!row.protected_legacy&&["connected","needs_attention"].includes(row.status)){var verify=el("button","button","Verificar");verify.type="button";verify.addEventListener("click",function(){adminChannelAction(row,"verify",false);});actions.appendChild(verify);}if(!row.protected_legacy&&["needs_attention","disconnected","not_connected"].includes(row.status)){var help=el("button","button","Ayudar a reconectar");help.type="button";help.addEventListener("click",function(){adminChannelAction(row,"help-reconnect",false);});actions.appendChild(help);}if(!row.protected_legacy&&["connected","needs_attention","connecting"].includes(row.status)){var disconnect=el("button","button danger","Desconectar");disconnect.type="button";disconnect.addEventListener("click",function(){adminChannelAction(row,"disconnect",true);});actions.appendChild(disconnect);}if(row.protected_legacy)actions.appendChild(el("span","badge neutral","Conexión RAV protegida"));line.append(client,channel,account,detail,actions);root.appendChild(line);});}
...
document.addEventListener("keydown",function(event){if(event.key==="Escape"){closeGoalEditor();closeTenant();if(customerAccessV2Enabled)closeCustomerCreate();}});
...
var nav=document.getElementById("leadNavCount");if(nav)nav.textContent=fmtNum(kpis.active||0);var count=document.getElementById("leadRowsCount");if(count)count.textContent=fmtNum(rows.length);
if(typeof paintClientCount==="function")paintClientCount(pipelineCustomerCount(),(leadsPipeline&&leadsPipeline.customers)||[]);
var root=document.getElementById("leadPipelineRows");if(!root)return;root.textContent="";if(!rows.length){root.appendChild(el("div","invite-loading","No hay leads activos. Cuando alguien cree cuenta con email y clave, aparecerá aquí."));return;}
...
var next=el("div");next.appendChild(el("small",null,row.next_action||"Revisar lead."));line.appendChild(next);root.appendChild(line);});}
...
function renderCustomerServiceConfiguration(root,configuration,review){var section=el("section","setup-config-editor");var head=el("div","question-toolbar");var title=el("div");title.appendChild(el("h2",null,"Borrador de Customer Service"));title.appendChild(el("p",null,"Generado desde el setup compartido. Appointment Setup queda reservado exclusivamente para el bot de agendamiento."));head.appendChild(title);head.appendChild(el("span","badge "+(configuration.lifecycle==="approved_for_testing"?"success":"warning"),configuration.lifecycle==="approved_for_testing"?"Aprobado para Testing":"Borrador interno"));section.appendChild(head);var fields=el("div","setup-review-fields");CUSTOMER_SERVICE_CONFIG_FIELDS.forEach(function(definition){fields.appendChild(setupConfigurationInput(definition,configuration));});section.appendChild(fields);var promptField=el("div","setup-review-field wide");promptField.appendChild(el("label",null,"Vista previa del prompt generado"));var prompt=document.createElement("textarea");prompt.readOnly=true;prompt.rows=18;prompt.value=configuration.system_prompt||"";promptField.appendChild(prompt);section.appendChild(promptField);var note=el("div","callout info-callout");note.appendChild(el("p",null,review.status==="testing"?"Esta versión ya fue aprobada para pruebas internas. Editarla la devolverá a Building y exigirá una nueva aprobación.":"Guardar cambios mantiene el bot en Building. Aprobar lo mueve a Testing, nunca a publicación pública."));section.appendChild(note);root.appendChild(section);}
function setupReviewErrorLabel(code){return({unauthorized:"Tu sesión expiró. Vuelve a entrar.",tenant_not_found:"Ese cliente ya no existe.",setup_not_completed:"El cliente todavía no ha terminado el setup.",setup_must_be_approved:"Primero aprueba el setup antes de construir.",customer_service_not_selected:"Este setup no incluye Customer Service. Las citas pertenecen al bot de agendamiento.",configuration_not_building:"Primero genera el borrador de Customer Service.",configuration_required:"La configuración está incompleta.",public_activation_requires_separate_approval:"La activación pública requiere una aprobación separada después de Testing.",setup_review_unavailable:"No se pudo cargar o guardar la revisión."})[code]||"No se pudo completar la revisión.";}
function renderSetupReviewRows(){var root=document.getElementById("setupReviewRows");if(!root)return;root.textContent="";if(!setupReviewCache.length){root.appendChild(el("div","invite-loading","Todavía no hay setups de clientes."));return;}setupReviewCache.forEach(function(row){var button=el("button","setup-review-row"+(setupReviewCurrent&&setupReviewCurrent.tenant&&setupReviewCurrent.tenant.id===row.tenant_id?" active":""),"");button.type="button";button.dataset.tenantId=row.tenant_id;button.appendChild(el("strong",null,row.company_name||row.tenant_id));button.appendChild(el("small",null,(row.tenant&&row.tenant.admin_email?row.tenant.admin_email+" · ":"")+setupGoalLabel(row.setup_goal)+" · "+(row.completion||0)+"% completo"));button.appendChild(setupStatusPill(row.review&&row.review.status));button.addEventListener("click",function(){openSetupReview(row.tenant_id);});root.appendChild(button);});}
...
function renderSetupReviewDetail(body){var root=document.getElementById("setupReviewDetail");if(!root)return;var tenant=body.tenant||{},record=body.onboarding||{},answers=record.answers||{},review=body.review||{},questions=setupReviewQuestions(body),answersApproved=setupAnswersApproved(answers);root.textContent="";
...
if(review.status==="incomplete"||(review.status==="ready"&&!answersApproved)){var request=el("button","button","Pedir cambios");request.type="button";request.addEventListener("click",function(){saveSetupReview("request_changes");});var approve=el("button","button primary","Aprobar setup");approve.type="button";approve.addEventListener("click",function(){saveSetupReview("approve");});actions.append(request,approve);}
if(review.status==="ready"&&answersApproved&&(answers.setup_goal==="customer_service"||answers.setup_goal==="both")){var build=el("button","button primary","Construir configuración");build.type="button";build.addEventListener("click",function(){saveSetupReview("build_configuration");});actions.appendChild(build);}
if(review.status==="building"&&record.customer_service_configuration){var saveConfig=el("button","button primary","Guardar configuración");saveConfig.type="button";saveConfig.addEventListener("click",function(){saveSetupReview("save_configuration");});var approveConfig=el("button","button","Aprobar para Testing");approveConfig.type="button";approveConfig.addEventListener("click",function(){saveSetupReview("approve_configuration");});actions.append(saveConfig,approveConfig);}
if(review.status==="testing"&&record.customer_service_configuration){var editConfig=el("button","button","Guardar cambios y volver a Building");editConfig.type="button";editConfig.addEventListener("click",function(){saveSetupReview("save_configuration");});actions.appendChild(editConfig);}
