...
function activateWhatsApp(button){if(state.whatsappActivating)return;if(DEMO_MODE){setChannelConnectionMessage("Demo: Nextfor verificaría la conexión una sola vez.","success");return;}state.whatsappActivating=true;if(button){button.disabled=true;button.textContent="Revisando…";}setChannelConnectionMessage("Verificando y activando WhatsApp con Meta…");api("/admin/panel/channel-connections/whatsapp/activate",{method:"POST",body:"{}"}).then(function(body){state.channelConnections=null;if(body.connection&&body.connection.status==="connected")setChannelConnectionMessage("Listo. WhatsApp quedó conectado y preparado para recibir mensajes.","success");else setChannelConnectionMessage("Meta dejó la conexión en proceso. No haremos más intentos hasta que vuelvas a revisarla.","success");loadChannelConnections(true);}).catch(function(error){state.channelConnections=null;setChannelConnectionMessage(error.body&&error.body.message||"Meta rechazó esta activación. Nextfor no hará más intentos automáticamente.","error");loadChannelConnections(true);}).finally(function(){state.whatsappActivating=false;if(button&&button.isConnected){button.disabled=false;button.textContent="Revisar conexión";}});}
