...
function botOpsFindingRow(row){var line=el("div","setup-review-row");var main=el("div");var title=el("strong",null,row.title||botOpsLabel(row.category));var company=row.company_name||row.tenant_id||"Empresa desconocida";var meta=el("small",null,company+" ("+(row.tenant_id||"sin tenant")+") · "+botOpsLabel(row.bot_id)+" · "+botOpsLabel(row.channel));main.append(title,meta);var status=el("div");status.appendChild(el("span","badge "+botOpsTone(row.severity),botOpsLabel(row.severity)));status.appendChild(el("small",null,"Último: "+botOpsWhen(row.last_seen_at)+" · "+fmtNum(row.occurrence_count||1)+" vez/veces"));var recommendation=el("div");recommendation.appendChild(el("strong",null,row.requires_approval?"Requiere aprobación":"Acción operativa"));recommendation.appendChild(el("small",null,row.recommendation||row.detail||"Revisar el hallazgo."));line.append(main,status,recommendation);return line;}
