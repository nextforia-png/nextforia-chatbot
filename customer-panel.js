...
@media(max-width:760px){
  ...
  .notificationControls{align-items:flex-start;flex-direction:column}.notificationControls .primaryBtn{width:100%}.notificationEmailHead,.notificationEmailActions{align-items:flex-start;flex-direction:column}.notificationEmailTypes{grid-template-columns:1fr}.notificationEmailActions .primaryBtn{width:100%}.handoffToast{right:16px;top:16px;grid-template-columns:42px 1fr}.handoffToast button{grid-column:1/-1;width:100%}
        ...
        <div class="notificationsView">
          ...
          <section class="notificationEmailSettings" id="notificationEmailSettings" hidden>
            ...
            <div class="notificationEmailTypes">
              <label class="notificationEmailType"><input type="checkbox" data-notification-email-type="payment_pending"> Pedidos y pagos por confirmar</label>
              <label class="notificationEmailType"><input type="checkbox" data-notification-email-type="shipping_pending"> Pedidos pendientes de envío</label>
              <label class="notificationEmailType"><input type="checkbox" data-notification-email-type="sales_opportunity"> Oportunidades de venta</label>
              <label class="notificationEmailType"><input type="checkbox" data-notification-email-type="product_update"> Novedades de Nextfor</label>
              <label class="notificationEmailType"><input type="checkbox" data-notification-email-type="human_attention"> Conversaciones que requieren atención</label>
            </div>
...
function showTab(name){
  ...
  if(name==="orders")loadOrders(false);if(name==="channels")loadChannelConnections(false);if(name==="setup"){loadBotSetup();loadBotPersonality(false);}if(name==="notifications"){renderNextforNotifications();loadNotificationEmailPreferences(false);}if(name==="retargeting"){loadRetargeting(false);if(!DEMO_MODE&&!state.ordersLoading&&!state.orders.length)loadOrders(false);}if(name==="appointments")loadAppointments();renderInbox();renderPlan();renderMobileDashboard();updateMobilePrimaryNav();window.scrollTo(0,0);
}
