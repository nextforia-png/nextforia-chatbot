...
function activateWhatsApp(button){if(state.whatsappActivating)return;if(DEMO_MODE){setChannelConnectionMessage("Demo: WhatsApp quedaría activo sin elegir otra cuenta.","success");return;}state.whatsappActivating=true;if(button){button.disabled=true;button.textContent="Activando…";}setChannelConnectionMessage("Activando el número en WhatsApp Cloud API…");api("/admin/panel/channel-connections/whatsapp/activate",{method:"POST",body:"{}"}).then(function(body){state.channelConnections=null;if(body.connection&&body.connection.status==="connected")setChannelConnectionMessage("Listo. WhatsApp quedó conectado y preparado para recibir mensajes.","success");else setChannelConnectionMessage("Meta aún está terminando la activación. Puedes reintentar aquí sin conectar otra cuenta.","success");loadChannelConnections(true);}).catch(function(error){setChannelConnectionMessage(error.body&&error.body.message||"No pudimos activar WhatsApp. Puedes reintentar sin conectar otra cuenta.","error");state.channelConnections=null;loadChannelConnections(true);}).finally(function(){state.whatsappActivating=false;if(button&&button.isConnected){button.disabled=false;button.textContent="Activar WhatsApp";}});}
...
function logoutCustomerPanel(){try{["rav_dashboard_key","rav_dashboard_tab","rav_logo","nextfor-integration-result","nextforia_tenant_id","tenant_id","rav_tenant_id"].forEach(function(key){localStorage.removeItem(key);sessionStorage.removeItem(key);});}catch(e){}fetch("/admin/logout",{method:"POST"}).finally(function(){location.replace("/admin/login");});}
