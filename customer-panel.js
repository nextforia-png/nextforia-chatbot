      ...
      <section class="${viewClass('panel-setup')}" id="panel-setup">
        <div class="setupView">
          <section class="setupSummaryPanel">
            ...
            <div class="channelPlan"><div><h4>Próximo paso: conectar canales y bot</h4><p id="channelConnectionSummary">Cuando el agente 🔌 Channels & Integrations esté listo, aquí se abrirá el flujo para conectar WhatsApp, Instagram y el bot correspondiente. Por ahora solo recopilamos la información.</p></div><button class="primaryBtn" type="button" disabled>Conexión pendiente</button></div>
...
function showTab(name){
  ...
  var pageTitle=name==="summary"?"Resumen":name==="tests"?"Pruebas":name==="plan"?"Mi plan":name==="setup"?"Configuración de tu Nextfor IA":name==="retargeting"?"Seguimientos comerciales":name==="appointments"?"Citas":"Conversaciones";
  var pageSubtitle=name==="summary"?("Resultados de "+(PANEL_CONTEXT.v2?PANEL_CONTEXT.assignedBotName:"tu bot de atención")+" · Últimos 7 días"):name==="tests"?"Herramientas seguras para validar el bot.":name==="plan"?"Plan, módulos y consumo":name==="setup"?"Tu negocio, tu voz y tus reglas en un solo lugar":name==="retargeting"?"Cola segura, aprobaciones, cancelaciones y auditoría":name==="appointments"?"Tu agenda llenándose, sin perseguir confirmaciones.":"La IA atiende y te deja solo lo que necesita de ti.";
