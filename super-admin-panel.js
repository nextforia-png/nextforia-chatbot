...
function appointmentGateLabel(status){return({ready:"Listo",needs_agent:"Falta agente",needs_webhook:"Falta webhook",needs_configuration:"Falta configurar agente",needs_provider:"Falta calendario",needs_customer_connection:"Debe conectar cuenta",oauth_not_configured:"Falta OAuth",manual_connection_required:"Conexión manual",needs_number:"Falta número",needs_email:"Falta correo",not_configured:"No configurado",not_requested:"No solicitado",optional:"Opcional",disabled:"Desactivado",blocked:"Bloqueado"}[status]||status||"Pendiente");}
...
if(review.status==="testing"&&(record.customer_service_configuration||record.appointment_configuration)){var editConfig=el("button","button","Guardar cambios y volver a Building");editConfig.type="button";editConfig.addEventListener("click",function(){saveSetupReview("save_configuration");});actions.appendChild(editConfig);if(record.appointment_configuration&&record.appointment_configuration.external_status!=="configured"){var configureAgent=el("button","button primary","Configurar agente real");configureAgent.type="button";configureAgent.addEventListener("click",function(){saveSetupReview("configure_appointment_agent","appointments");});actions.appendChild(configureAgent);}}
