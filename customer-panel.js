      ...
      <section class="${viewClass('panel-plan')}" id="panel-plan">
        <div class="planView">
          ${paymentGateRequired ? `<section class="planBlock">
            <h3>Autoriza Wompi para activar tu panel</h3>
            <p>Tu entrenamiento ya quedó guardado. El panel operativo se desbloquea cuando Wompi confirma el primer cobro de la suscripción o tu contrato queda listo por trial/piloto.</p>
            <div class="planCatalogNotice">Estás en el paso correcto: revisa la facturación y autoriza el cobro automático cuando esté disponible.</div>
          </section>` : ""}
          ...
          ${options.paymentsV1Enabled ? `<section class="planBlock" id="billingBlock">
            ...
            <p>La suscripción se confirma por webhook de Wompi. La respuesta del navegador por sí sola nunca activa tu suscripción.</p>
...
function startBillingCheckout(){var button=document.getElementById("billingPayButton");if(button){button.disabled=true;button.textContent="Preparando autorización…";}api("/admin/panel/billing/checkout",{method:"POST",body:JSON.stringify({plan_id:PANEL_CONTEXT.planId,bot_id:PANEL_CONTEXT.assignedBotId})}).then(function(body){if(!body.checkout||!body.checkout.authorization_url&&!body.checkout.checkout_url)throw new Error("authorization_unavailable");location.href=body.checkout.authorization_url||body.checkout.checkout_url;}).catch(function(error){if(button){button.disabled=false;button.textContent="Autorizar suscripción · Wompi Sandbox";}var root=document.getElementById("billingSummary");if(root)root.insertAdjacentHTML("afterbegin",'<div class="planCatalogNotice">'+esc("No pudimos abrir Wompi: "+error.message)+'</div>');});}
