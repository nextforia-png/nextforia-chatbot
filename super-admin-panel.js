...
function renderSuperAdminPanel(res, options) {
  ...
  const customerSetupQuestionnaire = options.customerSetupQuestionnaire && Array.isArray(options.customerSetupQuestionnaire.questions)
    ? options.customerSetupQuestionnaire
    : { version: 1, questions: [] };
  const questionnaireState = JSON.stringify(customerSetupQuestionnaire).replace(/</g, "\\u003c");
  const typeLabels = { text: "Texto corto", email: "Email", email_readonly: "Email solo lectura", tel: "Teléfono", textarea: "Texto largo", choice: "Opciones" };
  const questionRows = (customerSetupQuestionnaire.questions || []).map(function (question) {
    const typeOptions = ["text", "email", "email_readonly", "tel", "textarea", "choice"].map(function (type) {
      return '<option value="' + escapeHtml(type) + '"' + (question.type === type ? " selected" : "") + '>' + escapeHtml(typeLabels[type] || type) + '</option>';
    }).join("");
    return '<article class="question-row" data-question-id="' + escapeHtml(question.id) + '"><div class="question-main"><div class="question-top"><strong>' + escapeHtml(question.id) + '</strong><code>' + escapeHtml(question.path) + '</code><span>' + escapeHtml(question.section) + '</span></div><label>Pregunta<input data-question-field="label" value="' + escapeHtml(question.label || "") + '" maxlength="160"></label><label>Placeholder<input data-question-field="placeholder" value="' + escapeHtml(question.placeholder || "") + '" maxlength="280"></label></div><div class="question-controls"><label>Orden<input data-question-field="order" type="number" min="1" max="999" value="' + escapeHtml(question.order || 0) + '"></label><label>Tipo<select data-question-field="type">' + typeOptions + '</select></label><label class="mini-check"><input data-question-field="active" type="checkbox"' + (question.active !== false ? " checked" : "") + '>Activa</label><label class="mini-check"><input data-question-field="required" type="checkbox"' + (question.required === true ? " checked" : "") + '>Requerida</label></div></article>';
  }).join("");
...
.questionnaire-actions{display:flex;gap:9px;align-items:center;justify-content:flex-end;margin-top:14px}
.question-list{display:grid;gap:12px}
.question-row{display:grid;grid-template-columns:minmax(0,1fr) 280px;gap:14px;padding:15px;border:1px solid var(--border-subtle);border-radius:14px;background:var(--slate-50)}
.question-top{display:flex;gap:7px;align-items:center;flex-wrap:wrap;margin-bottom:9px}
.question-top strong{font:800 11px var(--font-mono);color:var(--text-strong)}
.question-top code{font:600 10px var(--font-mono);color:var(--cyan-700);background:#fff;border:1px solid var(--cyan-100);border-radius:6px;padding:3px 6px}
.question-top span{font-size:10px;color:var(--text-muted);font-weight:800;text-transform:uppercase;letter-spacing:.05em}
.question-row label{display:grid;gap:5px;font-size:10px;font-weight:850;color:var(--text-muted);text-transform:uppercase;letter-spacing:.04em}
.question-row input,.question-row select{width:100%;height:38px;border:1px solid var(--border-default);border-radius:10px;background:#fff;color:var(--text-strong);font:600 12px var(--font-body);padding:0 10px}
.question-main{display:grid;gap:10px}.question-controls{display:grid;grid-template-columns:1fr 1fr;gap:10px;align-content:start}
.mini-check{display:flex!important;align-items:center;gap:7px;height:38px;padding:0 10px;border:1px solid var(--border-default);border-radius:10px;background:#fff;text-transform:none!important;letter-spacing:0!important;color:var(--text-body)!important}
.mini-check input{width:15px;height:15px;padding:0;accent-color:var(--cyan-500)}
...
@media(max-width:1080px){.grid-4{grid-template-columns:repeat(2,1fr)}.attention{grid-template-columns:repeat(2,1fr)}.bot-grid,.two-col,.source-grid,.question-row{grid-template-columns:1fr}.top-actions .optional{display:none}}
...
<section class="view" data-panel="questionnaire"><div class="stack">
  <section class="card"><div class="card-head"><div><h2>Editor del cuestionario de setup</h2><p>Modifica cómo verá el Customer Admin las preguntas de configuración del bot. No llena respuestas del cliente ni crea un flujo paralelo.</p></div><span class="badge info">${(customerSetupQuestionnaire.questions || []).length} preguntas</span></div><div class="question-list" id="questionnaireRows">${questionRows}</div><div class="questionnaire-actions"><span class="health-value" id="questionnaireStatus">Cambios locales hasta guardar.</span><button class="button" type="button" onclick="loadQuestionnaire()">Recargar</button><button class="button primary" type="button" onclick="saveQuestionnaire()">Guardar cuestionario</button></div></section>
</div></section>
...
function qEsc(value){return String(value==null?"":value).replace(/[&<>"']/g,function(c){return{"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"}[c];});}
function qTypes(selected){var labels={text:"Texto corto",email:"Email",email_readonly:"Email solo lectura",tel:"Teléfono",textarea:"Texto largo",choice:"Opciones"};return["text","email","email_readonly","tel","textarea","choice"].map(function(type){return'<option value="'+qEsc(type)+'"'+(selected===type?" selected":"")+'>'+qEsc(labels[type])+'</option>';}).join("");}
function renderQuestionnaire(questionnaire){customerSetupQuestionnaire=questionnaire||customerSetupQuestionnaire||{questions:[]};var root=document.getElementById("questionnaireRows");if(!root)return;root.innerHTML=(customerSetupQuestionnaire.questions||[]).map(function(question){return'<article class="question-row" data-question-id="'+qEsc(question.id)+'"><div class="question-main"><div class="question-top"><strong>'+qEsc(question.id)+'</strong><code>'+qEsc(question.path)+'</code><span>'+qEsc(question.section)+'</span></div><label>Pregunta<input data-question-field="label" value="'+qEsc(question.label||"")+'" maxlength="160"></label><label>Placeholder<input data-question-field="placeholder" value="'+qEsc(question.placeholder||"")+'" maxlength="280"></label></div><div class="question-controls"><label>Orden<input data-question-field="order" type="number" min="1" max="999" value="'+qEsc(question.order||0)+'"></label><label>Tipo<select data-question-field="type">'+qTypes(question.type)+'</select></label><label class="mini-check"><input data-question-field="active" type="checkbox"'+(question.active!==false?" checked":"")+'>Activa</label><label class="mini-check"><input data-question-field="required" type="checkbox"'+(question.required===true?" checked":"")+'>Requerida</label></div></article>';}).join("");}
function collectQuestionnaire(){var byId={};(customerSetupQuestionnaire.questions||[]).forEach(function(question){byId[question.id]=question;});return{version:1,questions:[].slice.call(document.querySelectorAll(".question-row")).map(function(row){var id=row.getAttribute("data-question-id"),base=byId[id]||{};function value(name){var field=row.querySelector('[data-question-field="'+name+'"]');return field?field.type==="checkbox"?field.checked:field.value:"";}return{id:id,path:base.path,section:base.section,order:Number(value("order"))||base.order||0,active:value("active")===true,required:value("required")===true,type:value("type")||base.type,label:value("label"),placeholder:value("placeholder")};})};}
function loadQuestionnaire(){var status=document.getElementById("questionnaireStatus");if(status)status.textContent="Cargando…";fetch("/admin/customer-setup-questionnaire",{headers:{accept:"application/json"}}).then(function(response){return response.json().then(function(body){if(!response.ok)throw new Error(body.error||"questionnaire_unavailable");return body;});}).then(function(body){renderQuestionnaire(body.questionnaire);if(status)status.textContent="✓ Cuestionario actualizado desde servidor.";}).catch(function(error){if(status)status.textContent="No se pudo cargar: "+error.message;});}
function saveQuestionnaire(){var status=document.getElementById("questionnaireStatus"),payload=collectQuestionnaire();if(status)status.textContent="Guardando…";fetch("/admin/customer-setup-questionnaire",{method:"PUT",headers:{"content-type":"application/json",accept:"application/json"},body:JSON.stringify({questionnaire:payload})}).then(function(response){return response.json().then(function(body){if(!response.ok)throw new Error(body.message||body.error||"questionnaire_save_failed");return body;});}).then(function(body){renderQuestionnaire(body.questionnaire);if(status)status.textContent="✓ Cuestionario guardado. El Customer Admin verá estos cambios al abrir el setup.";showToast("Cuestionario guardado.");}).catch(function(error){if(status)status.textContent="No se pudo guardar: "+error.message;showToast("No se pudo guardar el cuestionario.");});}
...
if(document.activeElement&&document.activeElement.blur)document.activeElement.blur();button.disabled=true;button.textContent="Guardando…";errorBox.textContent="Guardando en servidor…";var controller=window.AbortController?new AbortController():null,timeout=setTimeout(function(){if(controller)controller.abort();},12000);
fetch("/admin/platform-goals/customers",{method:"PUT",credentials:"same-origin",cache:"no-store",headers:{"content-type":"application/json","accept":"application/json"},body:JSON.stringify({type:"counter",label:label,unit:"clientes",target:target,active:true}),signal:controller&&controller.signal}).then(function(response){return response.json().then(function(body){if(!response.ok)throw new Error(body.error||("http_"+response.status));return body;});}).then(function(body){paintPlatformGoal(body.goal);closeGoalEditor();showToast("Meta actualizada: "+target+" clientes.");}).catch(function(error){var message=error&&error.name==="AbortError"?"La conexión tardó demasiado. Reintenta en unos segundos.":error&&error.message==="invalid_goal_target"?"La meta debe ser un número entero válido.":"No se pudo guardar la meta ("+(error&&error.message||"error")+").";errorBox.textContent=message;}).finally(function(){clearTimeout(timeout);button.disabled=false;button.textContent="Guardar meta";});});
