...
@media(max-width:760px){
  ...
  body.customer-panel-mobile-v389.order-mobile-detail .orderShippingGrid button[data-field="id_number"]{display:none}
  body.customer-panel-mobile-v389.order-mobile-detail .orderShippingGrid strong{font-size:11.5px}
  body.customer-panel-mobile-v389.order-mobile-detail .orderTrackingCard,
...
function renderMobileOrderAction(order){if(order.stage!=="por_confirmar")return"";var disabled=SERVER_CAPABILITIES.manage_orders?"":" disabled";return '<div class="orderMobilePrimaryAction"><button data-order-action="confirm_payment" type="button" data-action="confirm_payment" onclick="orderAction(this.dataset.action)"'+disabled+'><span>✓</span> Confirmar pago recibido</button><small>Verifica el comprobante en el chat antes de confirmar.</small></div>';}
...
function enhanceMobileOrderPresentation(){
  ...
  if(shipping&&!detail.querySelector(".orderMobileOpenChat")){shipping.insertAdjacentHTML("afterend",'<button class="orderMobileOpenChat" type="button" onclick="openOrderConversation()">Abrir conversación <span>→</span></button>');}
}
