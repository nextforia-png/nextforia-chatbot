...
function connectAppointmentCalendar(){if(DEMO_MODE){setChannelConnectionMessage("Demo: aquí abriríamos Google para conectar el calendario de tu negocio.","success");return;}setChannelConnectionMessage("Abriendo Google Calendar…");api("/admin/panel/appointment-calendar/google/connect",{method:"POST",body:JSON.stringify({return_to:"channels"})}).then(function(body){if(!body.authorization_url)throw new Error("authorization_unavailable");location.assign(body.authorization_url);}).catch(function(error){setChannelConnectionMessage(error.body&&error.body.message||"No pudimos abrir Google Calendar. Intenta de nuevo o habla con NextforIA.","error");loadChannelConnections(true);});}
...
function notificationAction(url){url=String(url||"");if(!url)return;location.href=url;}
