...
assert(tenantA.includes('id="customerProfileEmail"'), "el perfil de escritorio debe editar el correo del cliente");
assert(tenantA.includes('id="customerProfileAddress"'), "el perfil de escritorio debe editar la dirección de envío");
assert(tenantA.includes('id="customerProfileAddressLine2"'), "el perfil de escritorio debe editar el complemento de la dirección");
assert(tenantA.includes('id="customerProfileNeighborhood"'), "el perfil de escritorio debe editar el barrio");
assert(tenantA.includes('id="customerProfileState"'), "el perfil de escritorio debe editar el departamento o estado");
assert(tenantA.includes('id="customerProfileCountry"'), "el perfil de escritorio debe editar el país");
assert(tenantA.includes('id="mobileCustomerProfilePhone"'), "el perfil móvil debe editar el teléfono del cliente");
assert(tenantA.includes('id="mobileCustomerProfileShippingNotes"'), "el perfil móvil debe editar las indicaciones de envío");
assert(tenantA.includes('body.panel-redesign.conversations-view.chat-open .mobileCustomerNameCard{display:block}'), "el editor de contacto debe permanecer accesible dentro del chat móvil");
...
assert(tenantA.includes("payload.name=customerNameValue()"), "el nombre confirmado debe enviarse al endpoint de metadata");
assert(tenantA.includes("profilePatch=customerProfilePayload(),payload={}"), "el perfil de contacto debe usar el endpoint canónico de metadata");
assert(tenantA.includes("payload.profile=profilePatch"), "la UI debe enviar el perfil modificado al endpoint canónico");
assert(tenantA.includes("Object.keys(state.metaDirtyFields||{})"), "la UI debe enviar solo campos modificados para preservar datos capturados por el bot");
assert(tenantA.includes("item.customer_profile=(response.meta&&response.meta.profile)||{}"), "la UI debe aplicar el perfil confirmado por backend");
