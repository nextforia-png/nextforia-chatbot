...
<div class="app">
  ...
  <aside class="sidebar">
    ...
    <div class="whatsappCard">
      <strong><span class="statusDot"></span>WhatsApp conectado</strong>
      <p id="whatsappStatus">+57 300 214 88 · Plan Growth</p>
    </div>
  </aside>
  <main class="main">
    <header class="topbar">
...
<script>
var INITIAL_TAB=${safeJson(initialTab)},SERVER_ROLE=${safeJson(auth.role)},SERVER_CAPABILITIES=${safeJson(capabilities)},PANEL_DATA_PATH=${safeJson(dataPath)},PANEL_HEALTH_PATH=${safeJson(healthPath)},PANEL_LOGIN_PATH=${safeJson(loginPath)};
...
var state={tab:INITIAL_TAB,filter:"all",data:null,health:null,conversations:[],selected:null,metaDirty:false,draftTags:[],loading:false,autopilot:true,suggestion:""};
...
function findConversation(phone){return state.conversations.find(function(item){return item.phone===phone;})||null;}
...
function matchesConversation(item,query){if(!query)return true;var messages=(item.messages||[]).map(function(m){return m.text||"";}).join(" "),tags=(item.tags||[]).map(tagLabel).join(" "),haystack=[item.phone,item.note,item.last_text,messages,tags].join(" ").toLowerCase();return haystack.indexOf(query)>=0;}
...
function renderThreads(){var box=document.getElementById("threadList");if(!box)return;var human=isHumanTab(),items=filteredConversations();box.innerHTML=items.length?items.map(function(item){var classes="thread"+(item.phone===state.selected?" active":"")+(item.needs_reply?" pending":"");var tags=(item.tags||[]).slice(0,3).map(function(tag){return '<span class="tag">'+esc(tagLabel(tag))+'</span>';}).join("");var wait=waitMinutes(item),hot=wait>=6,reason=human?'<div class="threadReason"><span>'+esc(handoffReason(item))+'</span><span class="waitPill'+(hot?' hot':'')+'">⏱️ '+wait+' min</span></div>':"";return '<button type="button" class="'+classes+'" data-phone="'+attr(item.phone)+'" onclick="selectConversation(this.dataset.phone)"><div class="threadTop"><strong>+'+esc(item.phone)+'</strong><time>'+esc(when(item.last_ts))+'</time></div><p>'+esc(item.last_text||"Sin mensajes")+'</p>'+reason+'<div class="tags"><span class="tag">'+(item.mode==="human"?"Humano":"Bot")+'</span>'+(item.needs_reply?'<span class="tag">Pendiente</span>':"")+tags+'</div></button>';}).join(""):'<div class="empty">'+(human?"No hay casos esperando intervención. El bot lo tiene bajo control 👌":"No hay conversaciones en este filtro.")+'</div>';}
function selectConversation(phone){state.selected=phone;state.metaDirty=false;var item=findConversation(phone);state.draftTags=item?(item.tags||[]).slice():[];renderThreads();renderChat();document.body.classList.add("chat-open");window.scrollTo(0,0);}
