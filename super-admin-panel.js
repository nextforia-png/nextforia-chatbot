...
function setupStatusHelp(status){return({incomplete:"Falta información o NextforIA pidió cambios al cliente.",ready:"El setup está completo y aprobado para construir.",building:"El bot está en construcción interna.",testing:"El bot está en pruebas antes de activarse.",live:"El bot está activo para el cliente."})[status]||"Estado de revisión pendiente.";}
...
function setupReviewErrorLabel(code){return({unauthorized:"Tu sesión expiró. Vuelve a entrar.",tenant_not_found:"Ese cliente ya no existe.",setup_review_unavailable:"No se pudo cargar o guardar la revisión."})[code]||"No se pudo completar la revisión.";}
...
function renderSetupReviewDetail(body){var root=document.getElementById("setupReviewDetail");if(!root)return;var tenant=body.tenant||{},record=body.onboarding||{},answers=record.answers||{},review=body.review||{},questions=setupReviewQuestions(body);root.textContent="";
...
var quick=el("div","callout info-callout");quick.style.marginTop="0";var quickText=el("div");quickText.appendChild(el("strong",null,setupStatusHelp(review.status)));quickText.appendChild(el("p",null,"Edita aquí el mismo setup que ve Customer Panel. Al guardar, queda una nueva versión auditada del record del tenant."));quick.appendChild(quickText);quick.appendChild(el("span","badge info",String(record.completion||0)+"% completo"));root.appendChild(quick);
var controls=el("div","setup-review-fields");var statusField=el("div","setup-review-field");statusField.appendChild(el("label",null,"Estado"));var statusSelect=document.createElement("select");statusSelect.id="setupReviewStatus";SETUP_REVIEW_STATUSES.forEach(function(item){var option=document.createElement("option");option.value=item[0];option.textContent=item[1];statusSelect.appendChild(option);});statusSelect.value=review.status||"incomplete";statusField.appendChild(statusSelect);controls.appendChild(statusField);
...
var actions=el("div","setup-review-actions");var save=el("button","button primary","Guardar revisión");save.type="button";save.addEventListener("click",function(){saveSetupReview("update");});var request=el("button","button","Pedir cambios");request.type="button";request.addEventListener("click",function(){saveSetupReview("request_changes","incomplete");});var approve=el("button","button","Aprobar setup");approve.type="button";approve.addEventListener("click",function(){saveSetupReview("approve","ready");});actions.append(save,request,approve);root.appendChild(actions);
...
function collectSetupReviewPayload(action,forcedStatus){if(!setupReviewCurrent)return null;var answers=cloneSetupAnswers(setupReviewCurrent.onboarding&&setupReviewCurrent.onboarding.answers);document.querySelectorAll("#setupReviewDetail [data-review-path]").forEach(function(input){setAnswerByPath(answers,input.dataset.reviewPath,input.type==="checkbox"?input.checked:input.value);});return{answers:answers,review_status:forcedStatus||document.getElementById("setupReviewStatus").value,review_note:document.getElementById("setupReviewNote").value,requested_changes:document.getElementById("setupReviewChanges").value,action:action||"update"};}
function saveSetupReview(action,forcedStatus){if(!setupReviewCurrent||!setupReviewCurrent.tenant)return;var payload=collectSetupReviewPayload(action,forcedStatus);if(action==="request_changes"&&!String(payload.requested_changes||"").trim()){document.getElementById("setupReviewChanges").focus();showToast("Escribe primero qué cambios debe hacer el cliente.");return;}var buttons=document.querySelectorAll("#setupReviewDetail button");buttons.forEach(function(button){button.disabled=true;});fetch("/admin/customer-setups/"+encodeURIComponent(setupReviewCurrent.tenant.id),{method:"PUT",headers:{"content-type":"application/json",accept:"application/json"},body:JSON.stringify(payload)}).then(function(response){return response.json().then(function(body){if(!response.ok)throw new Error(body.error||"setup_review_unavailable");return body;});}).then(function(body){setupReviewCurrent.onboarding=body.onboarding;setupReviewCurrent.review=body.review;renderSetupReviewDetail(setupReviewCurrent);showToast(action==="request_changes"?"Cambios solicitados. El cliente podrá volver al setup.":forcedStatus==="ready"?"Setup aprobado.":"Revisión guardada.");return loadSetupReviews();}).catch(function(error){showToast(setupReviewErrorLabel(error.message));}).finally(function(){document.querySelectorAll("#setupReviewDetail button").forEach(function(button){button.disabled=false;});});}
