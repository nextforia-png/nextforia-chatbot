...
<section class="view" data-panel="aiCosts"><div class="stack">
  <div class="callout info-callout" style="margin-top:0"><div><strong>Costos reales y consumo de inteligencia artificial</strong><p>NextforIA mide cada llamada por cliente, conversación, bot y función. El total oficial aparece cuando se configura una llave administrativa del proveedor. Nunca se muestran llaves, prompts ni datos privados.</p></div><span class="badge info">Solo Super Admin</span></div>
  <section class="card"><div class="card-head"><div><h2>Periodo analizado</h2><p id="aiCostPeriodNote">Elige un periodo y actualiza los datos.</p></div><div class="ai-cost-controls"><label class="sr-only" for="aiCostDays">Periodo</label><select id="aiCostDays" onchange="loadAiCosts(false)"><option value="7">Últimos 7 días</option><option value="30" selected>Últimos 30 días</option><option value="90">Últimos 90 días</option></select><button class="button" type="button" onclick="loadAiCosts(true)">${icon("refresh", 15)} Actualizar</button></div></div><div class="grid-4"><article class="stat-card"><div class="stat-top"><span>Costo estimado</span><span class="icon-chip">${icon("dollar", 17)}</span></div><div class="stat-value" id="aiCostTotal">—</div><div class="stat-sub">USD · telemetría NextforIA</div></article><article class="stat-card"><div class="stat-top"><span>Llamadas IA</span><span class="icon-chip">${icon("activity", 17)}</span></div><div class="stat-value" id="aiCostCalls">—</div><div class="stat-sub" id="aiCostFailures">Cargando</div></article><article class="stat-card"><div class="stat-top"><span>Tokens de entrada</span><span class="icon-chip">${icon("message", 17)}</span></div><div class="stat-value" id="aiCostInput">—</div><div class="stat-sub" id="aiCostCache">incluye contexto y herramientas</div></article><article class="stat-card"><div class="stat-top"><span>Tokens de salida</span><span class="icon-chip">${icon("bot", 17)}</span></div><div class="stat-value" id="aiCostOutput">—</div><div class="stat-sub">respuestas generadas</div></article></div></section>
  ...
  <section class="card table-card"><div class="list-head"><div><h2>Costo por cliente y bot</h2><span class="badge info" id="aiBotCount">0</span></div></div><div class="table-scroll"><div class="ai-cost-table-head"><span>Cliente</span><span>Bot</span><span>Llamadas</span><span>Tokens</span><span>Costo estimado</span></div><div id="aiBotRows"><div class="invite-loading">Cargando bots…</div></div></div></section>
  <section class="card table-card"><div class="list-head"><div><h2>Conversaciones que más consumen</h2><span class="badge neutral">identidad protegida</span></div></div><div class="table-scroll"><div class="ai-cost-table-head"><span>Conversación</span><span>Cliente / bot</span><span>Llamadas</span><span>Qué consumió</span><span>Costo estimado</span></div><div id="aiConversationRows"><div class="invite-loading">Cargando conversaciones…</div></div></div></section>
  <div class="two-col" style="margin-top:0"><section class="card"><div class="card-head"><div><h2>Qué está generando el consumo</h2><p>Funciones ordenadas por costo estimado.</p></div></div><div id="aiDriverRows" class="ai-cost-explanation"><div class="invite-loading">Cargando causas…</div></div></section><section class="card"><div class="card-head"><div><h2>Cómo leer este módulo</h2><p>Sin falsos estados de saldo.</p></div></div><div class="ai-cost-explanation"><div><strong>Activo</strong><span>NextforIA registró una llamada exitosa real durante el periodo.</span></div><div><strong>Sin créditos</strong><span>El proveedor rechazó una llamada real por saldo o cuota. Requiere recarga.</span></div><div><strong>Sin actividad comprobada</strong><span>La llave existe, pero no hubo una llamada real en el periodo. No gastamos tokens solo para probar.</span></div><div><strong>Costo estimado vs. oficial</strong><span>La conversación usa precios por token; el proveedor entrega el total facturado de la organización.</span></div></div></section></div>
</div></section>
...
function aiStatus(value){return({active:["Activo","success"],no_credits:["Sin créditos","danger"],missing_key:["Sin llave API","danger"],reporting_credentials_invalid:["Llave administrativa inválida","danger"],not_observed:["Sin actividad comprobada","warning"]})[value]||["Revisar","warning"];}
function aiEmpty(root,message){if(!root)return;root.textContent="";root.appendChild(el("div","invite-loading",message));}
function renderAiProviders(providers){var root=document.getElementById("aiProviderCards");if(!root)return;root.textContent="";(providers||[]).forEach(function(provider){var card=el("section","card ai-provider-card"),head=el("div","ai-provider-title"),title=el("h2",null,aiProviderLabel(provider.provider)),state=aiStatus(provider.operational_status);head.append(title,el("span","badge "+state[1]+" dot",state[0]));card.appendChild(head);var local=provider.local||{},official=provider.official||{},metrics=el("div","ai-cost-metrics");function metric(label,value,help){var item=el("div","ai-cost-metric");item.append(el("span",null,label),el("strong",null,value),el("small",null,help));return item;}metrics.append(metric("Costo por conversaciones",aiUsd(local.estimated_cost_usd),"estimación NextforIA"),metric("Costo oficial",official.status==="connected"?aiUsd(official.cost_usd):"No conectado",official.status==="connected"?"reporte del proveedor":provider.admin_reporting_configured?"error al consultar":"falta llave administrativa"),metric("Llamadas",String(local.calls||0),(local.failed_calls||0)+" fallidas"),metric("Última llamada exitosa",aiDate(local.last_success_at),local.no_credit_errors?local.no_credit_errors+" error(es) de crédito":"sin errores de crédito"));card.appendChild(metrics);var actions=el("div","ai-provider-actions"),usage=el("a","button","Ver consumo oficial"),recharge=el("a","button primary","Recargar / facturación");usage.href=provider.provider==="anthropic"?"https://platform.claude.com/usage":"https://platform.openai.com/usage";usage.target="_blank";usage.rel="noopener noreferrer";recharge.href=provider.provider==="anthropic"?"https://platform.claude.com/settings/billing":"https://platform.openai.com/settings/organization/billing/overview";recharge.target="_blank";recharge.rel="noopener noreferrer";actions.append(usage,recharge);card.appendChild(actions);root.appendChild(card);});if(!(providers||[]).length)aiEmpty(root,"No hay proveedores configurados.");}
...
function renderAiCosts(body){var totals=body&&body.usage&&body.usage.totals||{};document.getElementById("aiCostTotal").textContent=aiUsd(totals.estimated_cost_usd);document.getElementById("aiCostCalls").textContent=String(totals.calls||0);document.getElementById("aiCostFailures").textContent=(totals.failed_calls||0)+" fallidas · "+(totals.no_credit_errors||0)+" por créditos";document.getElementById("aiCostInput").textContent=aiTokens(totals.input_tokens);document.getElementById("aiCostCache").textContent="caché leído/escrito: "+aiTokens((totals.cached_input_tokens||0)+(totals.cache_write_tokens||0));document.getElementById("aiCostOutput").textContent=aiTokens(totals.output_tokens);document.getElementById("aiCostPeriodNote").textContent="Desde "+aiDate(body.since)+" · fuente "+(body.source==="supabase"?"persistente":"memoria temporal")+(body.complete?"":" · resultado parcial");renderAiProviders(body.providers||[]);renderAiBotRows(body);renderAiConversationRows(body);renderAiDrivers(body);}
