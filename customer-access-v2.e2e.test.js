...
(async function run() {
  ...
  try {
    ...
    assert(catalogs.plans.some(function (row) { return row.id === "growth"; }));
    ...
    response = await fetch(base + "/admin/customer-invite", {
      ...
      body: JSON.stringify({ company_name: "Empresa bloqueada", admin_email: "blocked@example.com", plan_id: "growth", assigned_bot_id: "atencion-cliente" })
    });
    ...
    response = await fetch(base + "/admin/customer-invite", {
      ...
      body: JSON.stringify({ company_name: "Empresa incompleta", admin_email: "incomplete@example.com", plan_id: "growth" })
    });
    assert.strictEqual(response.status, 400);
    assert.strictEqual((await response.json()).error, "invalid_request");

    const createPayload = { company_name: "Empresa Staging", admin_email: " Admin@Staging.Example ", plan_id: "growth", assigned_bot_id: "atencion-cliente" };
    response = await fetch(base + "/admin/customer-invite", {
      method: "POST",
      headers: { "content-type": "application/json", cookie: superCookie, origin: base },
    ...
    response = await fetch(base + "/admin/customer-invite", {
      method: "POST",
      headers: { "content-type": "application/json", cookie: superCookie, origin: base },
      body: JSON.stringify({ company_name: "Otra empresa", admin_email: "admin@staging.example", plan_id: "growth", assigned_bot_id: "atencion-cliente" })
    });
