  ...
  <aside class="sidebar">
    ...
    <div class="sidebarFoot">
      ...
      <div class="whatsappCard">
        <div class="botsActive"><span class="statusDot"></span><span>${panelRedesignEnabled ? "2 bots entrenados y listos" : activeBotCount + " " + (activeBotCount === 1 ? "bot activo" : "bots activos")}</span></div>
        <strong><span class="statusDot" id="channelStatusDot"></span><span id="channelStatusTitle">${panelRedesignEnabled ? "Bot de atención conectado" : (panelContext.v2 ? "Bot conectado" : "Bot de atención conectado")}</span></strong>
        ...
        <div class="planView">
          ...
          <section class="planHero${panelRedesignEnabled ? " approvedPlanHero" : ""}">
            <div>
              ...
              <h3 id="planName">${panelRedesignEnabled ? "Nextfor Aura" : "Bot " + escapeHtml(panelContext.v2 ? panelContext.assignedBotName : "Atención al cliente")}</h3>
              <p>${panelRedesignEnabled ? "Atiende, orienta y vende por tus canales. Tu equipo conserva el control cuando hace falta." : "Tu asistente de Nextfor IA está atendiendo clientes 24/7. Aquí ves tu plan, consumo y caminos para crecer sin sorpresas."}</p>
            ...
            ${panelRedesignEnabled ? '<div class="approvedPlanVisual"><img src="/admin/assets/lumen-plan-aura.png" alt="Lumen Aura"><article><small>Tu plan incluye</small><strong>Atención multicanal</strong><span>WhatsApp · Instagram · Messenger</span><strong>Panel y memoria de clientes</strong><span>Configuración y soporte base</span></article></div>' : ""}
...
function saveProfile(){var button=document.getElementById("profileSaveBtn"),name=document.getElementById("profileNameInput"),admin=document.getElementById("profileAdminName"),phone=document.getElementById("profilePhone"),value=name&&(name.value||"").trim(),profile={business_name:value,administrator_name:admin&&(admin.value||"").trim(),contact_phone:phone&&(phone.value||"").trim(),logo_data_url:state.accountLogo||""};if(!value){profileStatus("Escribe el nombre del negocio.",true);if(name)name.focus();return;}if(DEMO_MODE&&PANEL_REDESIGN_ENABLED){try{localStorage.setItem("nx_demo_profile_v1",JSON.stringify(profile));}catch(_){}fillAccountProfile({can_edit:true,profile:Object.assign({administrator_email:"demo@nextforia.com"},profile)});PANEL_CONTEXT.businessName=value;text("brandBusinessName",value);text("mobileBrandBusinessName",value);applyPanelLogo(state.accountLogo,value);profileStatus("Perfil demo actualizado en este navegador.",false);panelToast("Perfil demo actualizado. No se modificó ninguna cuenta real.");return;}if(button){button.disabled=true;button.textContent="Guardando…";}profileStatus("Guardando cambios…",false);api(PANEL_ACCOUNT_PATH,{method:"PUT",body:JSON.stringify(profile)}).then(function(payload){fillAccountProfile(payload);PANEL_CONTEXT.businessName=value;text("brandBusinessName",value);text("mobileBrandBusinessName",value);applyPanelLogo(state.accountLogo,value);profileStatus("Perfil actualizado.",false);}).catch(function(error){profileStatus(error.body&&error.body.message||"No pudimos guardar el perfil.",true);}).finally(function(){if(button){button.disabled=false;button.textContent="Guardar cambios";}});}
...
function renderActivity(items){var box=document.getElementById("activityChart");if(!box)return;if(!items.length){items=[34,43,58,37,74,88,61].map(function(messages,index){return{day:String(index),messages:messages};});}var max=Math.max.apply(null,items.map(function(i){return i.messages||0;}))||1;var pts=items.map(function(i,idx){var x=items.length===1?350:idx*(700/(items.length-1));var y=190-((i.messages||0)/max*150);return [x,y];}),d=PANEL_REDESIGN_ENABLED?smoothChartPath(pts):pts.map(function(p,i){return (i?"L":"M")+p[0]+" "+p[1];}).join(" ");box.innerHTML='<svg viewBox="0 0 700 220" preserveAspectRatio="none"><defs><linearGradient id="activityFill" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="#12A8F4" stop-opacity=".32"/><stop offset="1" stop-color="#12A8F4" stop-opacity=".02"/></linearGradient></defs><path class="activityArea" d="'+d+' L700 220 L0 220 Z" fill="url(#activityFill)"/><path class="activityLine" d="'+d+'" fill="none" stroke="#12A8F4" stroke-width="5" stroke-linecap="round" stroke-linejoin="round"/></svg>';text("activityRange","+18% vs. período anterior");}
...
function clearPanelBrowserState(){try{["rav_dashboard_key","rav_dashboard_tab","rav_logo","nextfor-integration-result"].forEach(function(key){localStorage.removeItem(key);});}catch(_){}try{["nextforia_tenant_id","tenant_id","rav_tenant_id"].forEach(function(key){sessionStorage.removeItem(key);});}catch(_){}}
function logoutCustomerPanel(){if(DEMO_MODE&&PANEL_REDESIGN_ENABLED){panelToast("Esta es una demo pública: no hay una sesión real que cerrar.");return;}clearPanelBrowserState();fetch("/admin/logout",{method:"POST"}).finally(function(){clearPanelBrowserState();location.href="/admin/login";});}
