...
module.exports = function renderCustomerPanel(res, options) {
  ...
  const ordersNav = ordersEnabled ? '<button class="navItem" id="nav-orders" type="button" onclick="showTab(\'orders\')"><span class="navIcon">' + PANEL_ICONS.package + '</span><span>Pedidos</span></button>' : "";
  const ordersMobileNav = ordersEnabled ? '<button id="mnav-orders" data-bot="support" type="button" onclick="showTab(\'orders\')"><span class="mobileNavIcon">' + PANEL_ICONS.package + '</span><span>Pedidos</span></button>' : "";
...
.contactCard{padding:15px 16px;border-radius:16px;background:#fff;border:1px solid var(--line)}
.contactCard h4{font-size:14px;font-weight:950;color:var(--navy-800)}
.contactCard>p{margin-top:5px;color:var(--slate-500);font-size:11px;line-height:1.4}
.customerContactGrid{display:grid;grid-template-columns:1fr 1fr;gap:9px;margin-top:11px}
.customerContactField{display:grid;gap:5px;min-width:0;color:var(--slate-500);font-size:10px;font-weight:900;text-transform:uppercase;letter-spacing:.04em}
.customerContactField.wide{grid-column:1/-1}
.customerContactField input,.customerContactField textarea{width:100%;min-width:0;border:1px solid var(--slate-300);border-radius:11px;background:#fff;padding:9px 10px;color:var(--navy-800);font:700 12px/1.4 var(--font-body);text-transform:none;letter-spacing:0}
.customerContactField input{height:40px}.customerContactField textarea{min-height:62px;resize:vertical}
.customerContactField input:focus,.customerContactField textarea:focus{outline:3px solid rgba(18,168,244,.16);border-color:var(--cyan-400)}
...
.mobileCustomerNameCard details>summary{display:flex;align-items:center;justify-content:space-between;gap:12px;color:var(--navy-800);font-size:13px;font-weight:950;cursor:pointer;list-style:none}
.mobileCustomerNameCard details>summary::-webkit-details-marker{display:none}
.mobileCustomerNameCard details>summary:after{content:"⌄";color:var(--cyan-600);font-size:18px}.mobileCustomerNameCard details[open]>summary:after{transform:rotate(180deg)}
.mobileCustomerNameCard .customerContactGrid{grid-template-columns:1fr;margin-top:10px}
.mobileCustomerNameCard .customerContactField.wide{grid-column:auto}
.mobileCustomerNameCard .customerContactField input,.mobileCustomerNameCard .customerContactField textarea{margin-top:0}
...
.relationshipCard,.customerFacts,.aiUnderstood,.nameCard,.contactCard,.noteCard{margin:0;border:0;border-bottom:1px solid var(--line);border-radius:0;padding:18px 22px;background:#fff}
...
.panel-redesign .orderTrackingCard{display:grid;grid-template-columns:minmax(0,1fr) minmax(270px,.8fr);gap:16px;align-items:center}.panel-redesign .orderTrackingCard>div:first-child{display:grid;gap:3px}.panel-redesign .orderTrackingCard p{color:#71809A;font-size:10px}.panel-redesign .orderTrackingForm{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:7px}.panel-redesign .orderTrackingForm input{min-width:0;height:40px;border:1px solid #D8E0EB;border-radius:10px;padding:0 11px;color:#0A1836}.panel-redesign .orderTrackingForm button{height:40px;padding:0 13px;border:0;border-radius:10px;background:#0A96DF;color:#fff;font-size:10px;font-weight:850}.panel-redesign .orderTrackingForm button:disabled,.panel-redesign .orderDetailActions button:disabled{opacity:.45;cursor:not-allowed}
...
@media(max-width:760px){
  ...
  body.panel-redesign.conversations-view.chat-open .mobileCustomerNameCard{display:block}
    ...
    <div class="content">
      ...
      <section class="${viewClass('panel-inbox')}" id="panel-inbox">
        <div class="inboxShell">
          ...
          <section class="column chatColumn">
            ...
            <div class="conversationAction">
              <section class="mobileCustomerNameCard">
                <details>
                  <summary>Datos del cliente y del envío</summary>
                  <div class="customerContactGrid">
                    <label class="customerContactField wide"><span>Nombre</span><input id="mobileCustomerProfileName" maxlength="160" autocomplete="name" placeholder="Nombre del cliente" oninput="markMetaDirty()"></label>
                    <div class="nameSuggestion wide" id="mobileNameSuggestion" hidden><span>✦ Nextfor sugiere<strong id="mobileSuggestedCustomerName"></strong></span><button type="button" onclick="useSuggestedCustomerName(true)">Usar nombre</button></div>
                    <label class="customerContactField"><span>Teléfono</span><input id="mobileCustomerProfilePhone" maxlength="40" autocomplete="tel" inputmode="tel" placeholder="+57 300 000 0000" oninput="markMetaDirty()"></label>
                    <label class="customerContactField"><span>Correo</span><input id="mobileCustomerProfileEmail" maxlength="180" autocomplete="email" inputmode="email" placeholder="cliente@correo.com" oninput="markMetaDirty()"></label>
                    <label class="customerContactField"><span>Documento</span><input id="mobileCustomerProfileDocument" maxlength="60" autocomplete="off" placeholder="Cédula o NIT" oninput="markMetaDirty()"></label>
                    <label class="customerContactField"><span>Ciudad</span><input id="mobileCustomerProfileCity" maxlength="120" autocomplete="address-level2" placeholder="Ciudad" oninput="markMetaDirty()"></label>
                    <label class="customerContactField wide"><span>Dirección</span><input id="mobileCustomerProfileAddress" maxlength="500" autocomplete="street-address" placeholder="Dirección de entrega" oninput="markMetaDirty()"></label>
                    <label class="customerContactField wide"><span>Complemento</span><input id="mobileCustomerProfileAddressLine2" maxlength="240" autocomplete="address-line2" placeholder="Apartamento, torre o local" oninput="markMetaDirty()"></label>
                    <label class="customerContactField"><span>Barrio</span><input id="mobileCustomerProfileNeighborhood" maxlength="120" autocomplete="off" placeholder="Barrio" oninput="markMetaDirty()"></label>
                    <label class="customerContactField"><span>Departamento / Estado</span><input id="mobileCustomerProfileState" maxlength="120" autocomplete="address-level1" placeholder="Departamento" oninput="markMetaDirty()"></label>
                    <label class="customerContactField"><span>Código postal</span><input id="mobileCustomerProfilePostalCode" maxlength="24" autocomplete="postal-code" placeholder="Código postal" oninput="markMetaDirty()"></label>
                    <label class="customerContactField"><span>País</span><input id="mobileCustomerProfileCountry" maxlength="120" autocomplete="country-name" placeholder="País" oninput="markMetaDirty()"></label>
                    <label class="customerContactField wide"><span>Indicaciones de envío</span><textarea id="mobileCustomerProfileShippingNotes" maxlength="500" placeholder="Referencias o instrucciones para entregar" oninput="markMetaDirty()"></textarea></label>
                  </div>
                  <div class="mobileCustomerNameActions"><small id="mobileMetaHint">Guarda los datos útiles para esta conversación.</small><button class="ghostBtn" id="mobileSaveMetaBtn" type="button" onclick="saveCustomerMeta()">Guardar perfil</button></div>
                </details>
              </section>
              ${panelRedesignEnabled
          ...
          <aside class="column profileColumn" id="conversationProfile" aria-label="Información del cliente">${panelRedesignEnabled ? '<button class="profileDrawerClose" type="button" onclick="toggleConversationProfile(false)" aria-label="Cerrar información del cliente">×</button>' : ''}<div class="profile"><div class="profileIdentity"><span class="avatarChannelWrap"><span class="contactAvatar big" id="profileAvatar">—</span><span class="channelBadge floating large" id="profileChannelBadge" hidden></span></span><h3 id="profileName">Selecciona un cliente</h3><p id="profileContact">—</p><button class="copyContact" id="copyBtn" type="button" onclick="copyPhone()">Copiar número</button></div><section class="relationshipCard"><span id="relationshipEyebrow">Relación</span><div><strong id="relationshipValue">—</strong><small id="relationshipLabel"></small></div><p id="relationshipCopy">Selecciona una conversación para ver el valor que la IA está ayudando a construir.</p></section><div class="customerFacts" id="customerFacts"></div><section class="aiUnderstood"><span>✦ Lo que la IA entendió</span><strong id="aiIntent">Selecciona una conversación.</strong><div class="aiChips" id="aiChips"></div></section><section class="nameCard"><h4>Nombre del cliente</h4><input id="customerProfileName" maxlength="160" autocomplete="name" placeholder="Escribe el nombre" oninput="markMetaDirty()"><div class="nameSuggestion" id="nameSuggestion" hidden><span>✦ Nextfor sugiere<strong id="suggestedCustomerName"></strong></span><button type="button" onclick="useSuggestedCustomerName()">Usar nombre</button></div><p>Confírmalo o corrígelo. Nextfor lo recordará en futuras conversaciones.</p></section><section class="contactCard"><h4>Contacto y datos de envío</h4><div class="customerContactGrid"><label class="customerContactField"><span>Teléfono</span><input id="customerProfilePhone" maxlength="40" autocomplete="tel" inputmode="tel" placeholder="+57 300 000 0000" oninput="markMetaDirty()"></label><label class="customerContactField"><span>Correo</span><input id="customerProfileEmail" maxlength="180" autocomplete="email" inputmode="email" placeholder="cliente@correo.com" oninput="markMetaDirty()"></label><label class="customerContactField"><span>Documento</span><input id="customerProfileDocument" maxlength="60" autocomplete="off" placeholder="Cédula o NIT" oninput="markMetaDirty()"></label><label class="customerContactField"><span>Ciudad</span><input id="customerProfileCity" maxlength="120" autocomplete="address-level2" placeholder="Ciudad" oninput="markMetaDirty()"></label><label class="customerContactField wide"><span>Dirección</span><input id="customerProfileAddress" maxlength="500" autocomplete="street-address" placeholder="Dirección de entrega" oninput="markMetaDirty()"></label><label class="customerContactField wide"><span>Complemento</span><input id="customerProfileAddressLine2" maxlength="240" autocomplete="address-line2" placeholder="Apartamento, torre o local" oninput="markMetaDirty()"></label><label class="customerContactField"><span>Barrio</span><input id="customerProfileNeighborhood" maxlength="120" autocomplete="off" placeholder="Barrio" oninput="markMetaDirty()"></label><label class="customerContactField"><span>Departamento / Estado</span><input id="customerProfileState" maxlength="120" autocomplete="address-level1" placeholder="Departamento" oninput="markMetaDirty()"></label><label class="customerContactField"><span>Código postal</span><input id="customerProfilePostalCode" maxlength="24" autocomplete="postal-code" placeholder="Código postal" oninput="markMetaDirty()"></label><label class="customerContactField"><span>País</span><input id="customerProfileCountry" maxlength="120" autocomplete="country-name" placeholder="País" oninput="markMetaDirty()"></label><label class="customerContactField wide"><span>Indicaciones de envío</span><textarea id="customerProfileShippingNotes" maxlength="500" placeholder="Referencias o instrucciones para entregar" oninput="markMetaDirty()"></textarea></label></div></section><section class="noteCard"><h4>Nota interna</h4><textarea id="customerNote" placeholder="Agrega contexto útil para tu equipo…" oninput="markMetaDirty()"></textarea><button class="ghostBtn" id="saveMetaBtn" type="button" onclick="saveCustomerMeta()">Guardar perfil</button><p id="metaHint">Selecciona una conversación.</p></section></div></aside>
        </div>
      </section>
...
function showChannel(){state.channel="all";state.selected=null;state.metaDirty=false;state.metaDirtyFields={};applyChannelData();showTab("summary");renderChannelState();renderHeader();renderSummary();renderInbox();}
...
function showTab(name){
  ...
  if(name==="orders")loadOrders(false);if(name==="channels")loadChannelConnections(false);if(name==="setup"){loadBotSetup();loadBotPersonality(false);}if(name==="notifications")renderNextforNotifications();if(name==="retargeting"){loadRetargeting(false);if(!DEMO_MODE&&!state.ordersLoading&&!state.orders.length)loadOrders(false);}if(name==="appointments")loadAppointments();renderInbox();renderPlan();updateMobilePrimaryNav();window.scrollTo(0,0);
}
...
function loadOrders(force){if(!PANEL_REDESIGN_ENABLED)return;if(DEMO_MODE){renderOrders();return;}if(state.ordersLoading&&!force)return;state.ordersLoading=true;renderOrders();api(PANEL_ORDERS_PATH,{redirectOnAuth:true}).then(function(result){state.orders=result.orders||[];if(!state.selectedOrder||!state.orders.some(function(order){return orderKey(order)===String(state.selectedOrder);})){state.selectedOrder=state.orders[0]?orderKey(state.orders[0]):null;}renderOrders();if(state.retargeting)renderRetargeting();}).catch(function(error){panelToast((error.body&&error.body.message)||"No pudimos cargar tus pedidos. Intenta de nuevo.",true);}).finally(function(){state.ordersLoading=false;renderOrders();});}
function setOrderFilter(button){state.orderFilter=button&&button.dataset&&button.dataset.orderFilter||"all";var visibleOrders=state.orders.filter(function(order){return state.orderFilter==="all"||order.stage===state.orderFilter;});if(!visibleOrders.some(function(order){return orderKey(order)===String(state.selectedOrder);})){state.selectedOrder=visibleOrders.length?orderKey(visibleOrders[0]):null;}state.orderPane="list";document.body.classList.remove("order-detail-open");renderOrders();}
function selectOrder(id){state.selectedOrder=String(id||"");state.orderPane="detail";document.body.classList.add("order-detail-open");renderOrders();var detail=document.getElementById("orderDetailPane");if(detail)detail.scrollTop=0;}
function closeOrderDetail(){state.orderPane="list";document.body.classList.remove("order-detail-open");}
function updateDemoOrderAction(order,action,tracking){if(action==="confirm_payment")order.stage="pagado";else if(action==="start_preparation")order.stage="preparacion";else if(action==="mark_sent")order.stage="enviado";else if(action==="cancel")order.stage="cancelado";else if(action==="send_tracking"){order.tracking_number=tracking;order.guide=tracking;}renderOrders();panelToast("Pedido demo actualizado. No se modificó información real.");}
function orderAction(action){var order=state.orders.find(function(item){return orderKey(item)===String(state.selectedOrder);});if(!order||!SERVER_CAPABILITIES.manage_orders)return;var tracking=document.getElementById("orderTrackingInput"),trackingValue=tracking?tracking.value.trim():"";if(action==="send_tracking"&&!trackingValue){panelToast("Escribe el número de guía antes de enviarlo.",true);if(tracking)tracking.focus();return;}if(DEMO_MODE){updateDemoOrderAction(order,action,trackingValue);return;}document.querySelectorAll("[data-order-action]").forEach(function(button){button.disabled=true;});api("/admin/panel/orders/action",{method:"POST",body:JSON.stringify({order_id:order.id,action:action,tracking_number:trackingValue})}).then(function(result){var index=state.orders.findIndex(function(item){return orderKey(item)===orderKey(result.order);});if(index>=0)state.orders[index]=result.order;panelToast(action==="send_tracking"?"Guía entregada al cliente y guardada.":"Pedido actualizado.");renderOrders();}).catch(function(error){panelToast((error.body&&error.body.message)||"No pudimos actualizar el pedido.",true);renderOrders();});}
...
function orderShippingText(order){return ["Nombre: "+orderValue(order,"name"),"Cédula: "+orderValue(order,"id_number"),"Teléfono: "+orderValue(order,"phone"),"Correo: "+orderValue(order,"email"),"Dirección: "+orderValue(order,"address"),"Ciudad / barrio: "+orderValue(order,"city",order.location||"No registrado")].join("\\n");}
function copyOrderField(field,label){var order=state.orders.find(function(item){return orderKey(item)===String(state.selectedOrder);});if(!order)return;var value=field==="city"?(order.city||order.location):order[field];copyOrderText(value||"",label);}
...
function openOrderConversation(){var order=state.orders.find(function(item){return orderKey(item)===String(state.selectedOrder);});if(!order||!order.conversation_id){panelToast("Este pedido no tiene una conversación asociada.",true);return;}if(DEMO_MODE){showTab("conversations");panelToast("Demo: abrimos la bandeja de conversaciones.");return;}location.assign("/admin/panel?tab=conversations&conversation="+encodeURIComponent(order.conversation_id));}
function renderOrderSteps(order){if(order.stage==="cancelado")return '<div class="orderCanceledBanner"><strong>Pedido cancelado</strong><span>Se conserva el historial para tu equipo.</span></div>';var steps=[["Pedido",true],["Pago",["pagado","preparacion","enviado"].includes(order.stage)],["Preparación",["preparacion","enviado"].includes(order.stage)],["Enviado",order.stage==="enviado"]];return '<div class="orderStepper">'+steps.map(function(step,index){return '<div class="'+(step[1]?'done':'')+'"><i>'+(step[1]?'✓':index+1)+'</i><span>'+step[0]+'</span></div>';}).join("")+'</div>';}
...
function matchesConversation(item,query){if(!query)return true;var memory=item.memory||{},profile=item.customer_profile||{},messages=(item.messages||[]).map(function(m){return m.text||"";}).join(" "),tags=(item.tags||[]).map(tagLabel).join(" "),haystack=[item.phone,item.email,item.id,item.instagram_username,item.messenger_username,item.channel,item.channel_label,item.display_name,item.note,item.last_text,profile.phone,profile.email,profile.id_number,profile.address,profile.address_line_2,profile.neighborhood,profile.city,profile.state,profile.postal_code,profile.country,profile.delivery_instructions,messages,tags,memory.preferred_name,memory.purchase_stage,(memory.interests||[]).join(" ")].join(" ").toLowerCase();return haystack.indexOf(query)>=0;}
...
function selectConversation(key){state.selected=key;state.metaDirty=false;state.metaDirtyFields={};var item=findConversation(key);state.draftTags=item?(item.tags||[]).slice():[];state.guidedFor=key;state.guidedDraft=item&&uiStatus(item)==="you"?suggestedReply(item):"";document.body.classList.remove("profile-open");renderThreads();renderChat();document.body.classList.add("chat-open");window.scrollTo(0,0);}
...
function closeConversation(){closeEmojiPickers();state.selected=null;state.metaDirty=false;state.metaDirtyFields={};state.guidedFor=null;state.guidedDraft="";document.body.classList.remove("chat-open","profile-open");renderThreads();renderChat();window.scrollTo(0,0);}
...
var CUSTOMER_PROFILE_BINDINGS={phone:["customerProfilePhone","mobileCustomerProfilePhone"],email:["customerProfileEmail","mobileCustomerProfileEmail"],id_number:["customerProfileDocument","mobileCustomerProfileDocument"],address:["customerProfileAddress","mobileCustomerProfileAddress"],address_line_2:["customerProfileAddressLine2","mobileCustomerProfileAddressLine2"],neighborhood:["customerProfileNeighborhood","mobileCustomerProfileNeighborhood"],city:["customerProfileCity","mobileCustomerProfileCity"],state:["customerProfileState","mobileCustomerProfileState"],postal_code:["customerProfilePostalCode","mobileCustomerProfilePostalCode"],country:["customerProfileCountry","mobileCustomerProfileCountry"],delivery_instructions:["customerProfileShippingNotes","mobileCustomerProfileShippingNotes"]};
function customerProfileInputs(){return Object.keys(CUSTOMER_PROFILE_BINDINGS).reduce(function(all,key){return all.concat(CUSTOMER_PROFILE_BINDINGS[key].map(function(id){return document.getElementById(id);}).filter(Boolean));},[]);}
function customerProfileValue(key){var inputs=(CUSTOMER_PROFILE_BINDINGS[key]||[]).map(function(id){return document.getElementById(id);}).filter(Boolean),active=document.activeElement;if(active&&inputs.indexOf(active)>=0)return active.value.trim();var visible=inputs.find(function(input){return input.getBoundingClientRect().width>0;});return (visible||inputs[0])?(visible||inputs[0]).value.trim():"";}
function customerProfilePayload(){return Object.keys(state.metaDirtyFields||{}).reduce(function(profile,key){if(CUSTOMER_PROFILE_BINDINGS[key])profile[key]=customerProfileValue(key);return profile;},{});}
function fillCustomerProfile(profile,item){profile=profile||{};Object.keys(CUSTOMER_PROFILE_BINDINGS).forEach(function(key){var fallback=key==="phone"?(item&&item.contact_phone||(channelType(item)==="whatsapp"&&item&&item.phone)||""):(key==="email"?(item&&item.customer_email||item&&item.email||""):"");var value=profile[key]||fallback||"";CUSTOMER_PROFILE_BINDINGS[key].forEach(function(id){var input=document.getElementById(id);if(input)input.value=value;});});}
function syncCustomerProfileInput(active){Object.keys(CUSTOMER_PROFILE_BINDINGS).some(function(key){var ids=CUSTOMER_PROFILE_BINDINGS[key];if(ids.indexOf(active.id)<0)return false;ids.forEach(function(id){var input=document.getElementById(id);if(input&&input!==active)input.value=active.value;});return true;});}
...
function renderProfile(item,canMeta){
  var note=document.getElementById("customerNote"),names=customerNameInputs(),profileInputs=customerProfileInputs(),suggestions=[document.getElementById("nameSuggestion"),document.getElementById("mobileNameSuggestion")].filter(Boolean),chips=document.getElementById("aiChips"),facts=document.getElementById("customerFacts"),copy=document.getElementById("copyBtn");
  ...
  if(!item){
    ...
    if(chips)chips.innerHTML="";if(facts)facts.innerHTML="";if(note){note.value="";note.disabled=true;}names.forEach(function(name){name.value="";name.disabled=true;});profileInputs.forEach(function(input){input.value="";input.disabled=true;});suggestions.forEach(function(suggestion){suggestion.hidden=true;});setMetaSaveDisabled(true);if(copy)copy.disabled=true;setMetaHint("Selecciona una conversación.");return;
  }
  ...
  if(!state.metaDirty){if(note)note.value=item.note||"";names.forEach(function(name){name.value=item.customer_name||"";});fillCustomerProfile(item.customer_profile,item);}
  if(note)note.disabled=!canMeta;names.forEach(function(name){name.disabled=!canMeta;});profileInputs.forEach(function(input){input.disabled=!canMeta;});
...
function markMetaDirty(){if(!state.selected||!SERVER_CAPABILITIES.manage_notes_tags)return;var active=document.activeElement;if(active&&["customerProfileName","mobileCustomerProfileName"].includes(active.id)){state.metaDirtyFields.name=true;customerNameInputs().forEach(function(input){if(input!==active)input.value=active.value;});}else if(active&&active.id==="customerNote"){state.metaDirtyFields.note=true;}else if(active){Object.keys(CUSTOMER_PROFILE_BINDINGS).forEach(function(key){if(CUSTOMER_PROFILE_BINDINGS[key].indexOf(active.id)>=0)state.metaDirtyFields[key]=true;});syncCustomerProfileInput(active);}state.metaDirty=true;setMetaSaveDisabled(false);setMetaHint("Cambios sin guardar.");}
function saveCustomerMeta(){var item=findConversation(state.selected),note=document.getElementById("customerNote"),profilePatch=customerProfilePayload(),payload={};if(!item||!SERVER_CAPABILITIES.manage_notes_tags)return;if(Object.keys(profilePatch).length)payload.profile=profilePatch;if(state.metaDirtyFields.name)payload.name=customerNameValue();if(state.metaDirtyFields.note)payload.note=note?note.value.trim():"";setMetaSaveDisabled(true);setMetaHint("Guardando...");api("/admin/customer-meta/"+encodeURIComponent(conversationKey(item)),{method:"POST",body:JSON.stringify(payload)}).then(function(response){item.tags=(response.meta&&response.meta.tags)||item.tags||[];item.note=(response.meta&&response.meta.note)||"";item.customer_name=(response.meta&&response.meta.name)||null;item.customer_profile=(response.meta&&response.meta.profile)||{};item.display_name=item.customer_name||item.suggested_name||item.display_name;item.meta_updated_at=response.meta&&response.meta.updated_at;state.metaDirty=false;state.metaDirtyFields={};renderInbox();text("chatStatus","Perfil del cliente guardado.");loadPanelData(false);}).catch(function(error){setMetaHint((error.body&&error.body.message)||("No se pudo guardar: "+error.message));setMetaSaveDisabled(false);});}
