...
function updateDemoOrderAction(order,action,tracking,trackingUrl){if(action==="confirm_payment")order.stage="pagado";else if(action==="start_preparation")order.stage="preparacion";else if(action==="mark_sent")order.stage="enviado";else if(action==="cancel")order.stage="cancelado";else if(action==="send_tracking"){order.tracking_number=tracking;order.guide=tracking;order.tracking_url=trackingUrl;}renderOrders();panelToast("Pedido demo actualizado. No se modificó información real.");}
function orderAction(action){var order=state.orders.find(function(item){return orderKey(item)===String(state.selectedOrder);});if(!order||!SERVER_CAPABILITIES.manage_orders)return;var tracking=document.getElementById("orderTrackingInput"),trackingUrl=document.getElementById("orderTrackingUrlInput"),trackingValue=tracking?tracking.value.trim():"",trackingUrlValue=trackingUrl?trackingUrl.value.trim():"";if(action==="send_tracking"&&!trackingValue){panelToast("Escribe el número de guía antes de enviarlo.",true);if(tracking)tracking.focus();return;}if(action==="send_tracking"&&!trackingUrlValue){panelToast("Agrega el enlace de rastreo de la transportadora.",true);if(trackingUrl)trackingUrl.focus();return;}if(action==="send_tracking"&&trackingUrlValue.toLowerCase().indexOf("https://")!==0){panelToast("El enlace de rastreo debe comenzar por https://.",true);if(trackingUrl)trackingUrl.focus();return;}if(DEMO_MODE){updateDemoOrderAction(order,action,trackingValue,trackingUrlValue);return;}document.querySelectorAll("[data-order-action]").forEach(function(button){button.disabled=true;});api("/admin/panel/orders/action",{method:"POST",body:JSON.stringify({order_id:order.id,action:action,tracking_number:trackingValue,tracking_url:trackingUrlValue})}).then(function(result){var index=state.orders.findIndex(function(item){return orderKey(item)===orderKey(result.order);});if(index>=0)state.orders[index]=result.order;panelToast(action==="send_tracking"?"Guía y enlace entregados al cliente y guardados.":"Pedido actualizado.");renderOrders();}).catch(function(error){panelToast((error.body&&error.body.message)||"No pudimos actualizar el pedido.",true);renderOrders();});}
...
renderOrders=function(){renderOrdersBase();document.querySelectorAll(".orderRow").forEach(function(row){var order=state.orders.find(function(item){return orderKey(item)===String(row.dataset.order);}),value=row.querySelector("b");if(order&&value&&orderShippingPending(order))value.textContent=orderTotalLabel(order);});var selected=state.orders.find(function(order){return orderKey(order)===String(state.selectedOrder);}),trackingAllowed=selected&&["pagado","preparacion","enviado"].includes(selected.stage)&&SERVER_CAPABILITIES.manage_orders;document.querySelectorAll("#orderTrackingInput,#orderTrackingUrlInput,[data-action=send_tracking]").forEach(function(control){control.disabled=!trackingAllowed;});if(!selected||!orderShippingPending(selected))return;var productRows=document.querySelectorAll("#orderDetailPane .orderProducts>div");productRows.forEach(function(row){var label=row.querySelector("span"),value=row.querySelector("strong");if(!label||!value)return;if(label.textContent==="Envío")value.textContent=orderShippingLabel(selected);if(row.classList.contains("orderTotal")){label.textContent=orderTotalHeading(selected);value.textContent=orderTotalLabel(selected);}});};
