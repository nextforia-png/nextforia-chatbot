...
function activateWhatsApp(button,automatic){if(state.whatsappActivating)return;if(DEMO_MODE){if(!automatic)setChannelConnectionMessage("Demo: Nextfor terminaría la conexión automáticamente.","success");return;}if(!automatic)state.whatsappActivationPolls=0;state.whatsappActivating=true;if(button){button.disabled=true;button.textContent="Revisando…";}if(!automatic)setChannelConnectionMessage("Verificando y activando WhatsApp con Meta…");api("/admin/panel/channel-connections/whatsapp/activate",{method:"POST",body:"{}"}).then(function(body){state.channelConnections=null;if(body.connection&&body.connection.status==="connected"){state.whatsappActivationPolls=12;if(state.whatsappActivationTimer){clearTimeout(state.whatsappActivationTimer);state.whatsappActivationTimer=null;}setChannelConnectionMessage("Listo. WhatsApp quedó conectado y preparado para recibir mensajes.","success");}else{state.whatsappActivationPolls=(state.whatsappActivationPolls||0)+1;if(!automatic)setChannelConnectionMessage("Meta todavía está terminando la conexión. Nextfor seguirá verificando automáticamente.","success");}loadChannelConnections(true);}).catch(function(error){state.channelConnections=null;state.whatsappActivationPolls=(state.whatsappActivationPolls||0)+1;if(!automatic)setChannelConnectionMessage(error.body&&error.body.message||"No pudimos verificar la conexión. Puedes volver a autorizarla desde este panel.","error");loadChannelConnections(true);}).finally(function(){state.whatsappActivating=false;if(button&&button.isConnected){button.disabled=false;button.textContent="Revisar conexión";}scheduleWhatsAppActivationCheck();});}
